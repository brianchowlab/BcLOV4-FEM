...
if isa(surface1, 'triangulation') & ~isa(surface1, 'delaunayTriangulation')
  S = surface1;
  surface1 =[];
  %[surface1.faces, surface1.vertices] = freeBoundary(surface1);
  %[TR.ConnectivityList, TR.Points] = surface1;
...
if isa(surface2, 'triangulation') & ~isa(surface2, 'delaunayTriangulation')
  S = surface2;
  surface2 =[];
  %[surface1.faces, surface1.vertices] = freeBoundary(surface1);
  %[TR.ConnectivityList, TR.Points] = surface1;
...
for row = 1:size(N,1)
  ...
  if getIntersection && overlap(row)
    ...
    %if isempty(order) % when one tri is totally contained in the other tri then order is set
      %if size(uniquetol([P(edgeMat,:); squeeze(U(row,:,:))'],'ByRows',true),1) == 3
      %    disp('Possible Error')
      %    continue
      %end
      %if size(uniquetol([P(edgeMat,:); squeeze(V(row,:,:))'],'ByRows',true),1) == 3
      %    disp('Possible Error')
      %    continue
      %end
      %disp(points)
      %disp(uniquetol([P(edgeMat,:); squeeze(U(row,:,:))'],'ByRows',true))
...
%Patch for when a plane of the voxel is flush with the triangle
if key>432
    polygon = [];
    return
end
...
if nx==2
  ...
  if flip
    ...
    %disp(points(polygon,dIdx))
    %polygon
    if size(unique(points(polygon,dIdx),'rows'),1) < 3
        polygon = [];
        return
    end
