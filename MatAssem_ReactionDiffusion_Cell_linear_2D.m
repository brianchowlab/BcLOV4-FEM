function MATS = MatAssem_ReactionDiffusion_Cell_2D()
    %Define domain (2-D volume)
    Omega = Domain('triangle');
    
    %Define surface
    dOmega = Domain('interval') < Omega;

    %Define finite element spaces (in bulk and surface)
    C_h = Element(Omega, lagrange_deg1_dim2,1); % piecewise linear
    M_h = Element(dOmega,lagrange_deg1_dim1,1); % piecewise linear
    C_h_t = Element(Omega, lagrange_deg1_dim2,2); % piecewise linear
    M_h_t = Element(dOmega,lagrange_deg1_dim1,2); % piecewise linear
