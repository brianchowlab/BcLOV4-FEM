...
struct EDGE_DOF_PERMUTATION
{
    ...
    void Setup_Maps ()
        ...
        // DoF set #2, (current) -3 ---> (init) -3
        perm[1][2][0][2][0].map[1] = 1;
        perm[1][2][0][2][0].map[2] = 2;

        // DoF set #2, (current) -3 ---> (init) -2
        perm[1][2][0][1][0].map[1] = 1;
        perm[1][2][0][1][0].map[2] = 2;

        // DoF set #2, (current) -3 ---> (init) -1
        perm[1][2][0][0][0].map[1] = 1;
        perm[1][2][0][0][0].map[2] = 2;

        // DoF set #2, (current) -3 ---> (init) 1
        perm[1][2][0][0][1].map[1] = 2;
        perm[1][2][0][0][1].map[2] = 1;

        // DoF set #2, (current) -3 ---> (init) 2
        perm[1][2][0][1][1].map[1] = 2;
        perm[1][2][0][1][1].map[2] = 1;

        // DoF set #2, (current) -3 ---> (init) 3
        perm[1][2][0][2][1].map[1] = 2;
        perm[1][2][0][2][1].map[2] = 1;

        // DoF set #2, (current) -2 ---> (init) -3
        perm[1][1][0][2][0].map[1] = 1;
        perm[1][1][0][2][0].map[2] = 2;

        // DoF set #2, (current) -2 ---> (init) -2
        perm[1][1][0][1][0].map[1] = 1;
        perm[1][1][0][1][0].map[2] = 2;

        // DoF set #2, (current) -2 ---> (init) -1
        perm[1][1][0][0][0].map[1] = 1;
        perm[1][1][0][0][0].map[2] = 2;

        // DoF set #2, (current) -2 ---> (init) 1
        perm[1][1][0][0][1].map[1] = 2;
        perm[1][1][0][0][1].map[2] = 1;

        // DoF set #2, (current) -2 ---> (init) 2
        perm[1][1][0][1][1].map[1] = 2;
        perm[1][1][0][1][1].map[2] = 1;

        // DoF set #2, (current) -2 ---> (init) 3
        perm[1][1][0][2][1].map[1] = 2;
        perm[1][1][0][2][1].map[2] = 1;

        // DoF set #2, (current) -1 ---> (init) -3
        perm[1][0][0][2][0].map[1] = 1;
        perm[1][0][0][2][0].map[2] = 2;

        // DoF set #2, (current) -1 ---> (init) -2
        perm[1][0][0][1][0].map[1] = 1;
        perm[1][0][0][1][0].map[2] = 2;

        // DoF set #2, (current) -1 ---> (init) -1
        perm[1][0][0][0][0].map[1] = 1;
        perm[1][0][0][0][0].map[2] = 2;

        // DoF set #2, (current) -1 ---> (init) 1
        perm[1][0][0][0][1].map[1] = 2;
        perm[1][0][0][0][1].map[2] = 1;

        // DoF set #2, (current) -1 ---> (init) 2
        perm[1][0][0][1][1].map[1] = 2;
        perm[1][0][0][1][1].map[2] = 1;

        // DoF set #2, (current) -1 ---> (init) 3
        perm[1][0][0][2][1].map[1] = 2;
        perm[1][0][0][2][1].map[2] = 1;

        // DoF set #2, (current) 1 ---> (init) -3
        perm[1][0][1][2][0].map[1] = 2;
        perm[1][0][1][2][0].map[2] = 1;

        // DoF set #2, (current) 1 ---> (init) -2
        perm[1][0][1][1][0].map[1] = 2;
        perm[1][0][1][1][0].map[2] = 1;

        // DoF set #2, (current) 1 ---> (init) -1
        perm[1][0][1][0][0].map[1] = 2;
        perm[1][0][1][0][0].map[2] = 1;

        // DoF set #2, (current) 1 ---> (init) 1
        perm[1][0][1][0][1].map[1] = 1;
        perm[1][0][1][0][1].map[2] = 2;

        // DoF set #2, (current) 1 ---> (init) 2
        perm[1][0][1][1][1].map[1] = 1;
        perm[1][0][1][1][1].map[2] = 2;

        // DoF set #2, (current) 1 ---> (init) 3
        perm[1][0][1][2][1].map[1] = 1;
        perm[1][0][1][2][1].map[2] = 2;

        // DoF set #2, (current) 2 ---> (init) -3
        perm[1][1][1][2][0].map[1] = 2;
        perm[1][1][1][2][0].map[2] = 1;

        // DoF set #2, (current) 2 ---> (init) -2
        perm[1][1][1][1][0].map[1] = 2;
        perm[1][1][1][1][0].map[2] = 1;

        // DoF set #2, (current) 2 ---> (init) -1
        perm[1][1][1][0][0].map[1] = 2;
        perm[1][1][1][0][0].map[2] = 1;

        // DoF set #2, (current) 2 ---> (init) 1
        perm[1][1][1][0][1].map[1] = 1;
        perm[1][1][1][0][1].map[2] = 2;

        // DoF set #2, (current) 2 ---> (init) 2
        perm[1][1][1][1][1].map[1] = 1;
        perm[1][1][1][1][1].map[2] = 2;

        // DoF set #2, (current) 2 ---> (init) 3
        perm[1][1][1][2][1].map[1] = 1;
        perm[1][1][1][2][1].map[2] = 2;

        // DoF set #2, (current) 3 ---> (init) -3
        perm[1][2][1][2][0].map[1] = 2;
        perm[1][2][1][2][0].map[2] = 1;

        // DoF set #2, (current) 3 ---> (init) -2
        perm[1][2][1][1][0].map[1] = 2;
        perm[1][2][1][1][0].map[2] = 1;

        // DoF set #2, (current) 3 ---> (init) -1
        perm[1][2][1][0][0].map[1] = 2;
        perm[1][2][1][0][0].map[2] = 1;

        // DoF set #2, (current) 3 ---> (init) 1
        perm[1][2][1][0][1].map[1] = 1;
        perm[1][2][1][0][1].map[2] = 2;

        // DoF set #2, (current) 3 ---> (init) 2
        perm[1][2][1][1][1].map[1] = 1;
        perm[1][2][1][1][1].map[2] = 2;

        // DoF set #2, (current) 3 ---> (init) 3
        perm[1][2][1][2][1].map[1] = 1;
        perm[1][2][1][2][1].map[2] = 2;
                         ...
                         const int              DoF_Offset)
{
    ...
    if (Num_DoF_Per_Edge > 0)
        ...
        for (ei=Tri_Edge_Search->Edge_List.begin(); ei!=Tri_Edge_Search->Edge_List.end(); ++ei)
            ...
            // Set 2:
            edge_perm_map = Edge_DoF_Perm.Get_Map(2, current_local_edge_index, initial_local_edge_index);
            cell_dof[ Node.E[2][cei][1] ][current_cell_index] = SE_DoF[2][ edge_perm_map[1] ];
            cell_dof[ Node.E[2][cei][2] ][current_cell_index] = SE_DoF[2][ edge_perm_map[2] ];
            /*------------   END: Auto Generate ------------*/
            }
