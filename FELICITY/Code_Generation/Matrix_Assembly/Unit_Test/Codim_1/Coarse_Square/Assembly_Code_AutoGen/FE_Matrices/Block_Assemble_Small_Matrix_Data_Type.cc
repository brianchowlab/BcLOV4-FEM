...
#define SpecificFEM        Block_Assemble_Small_Matrix_Data_Type
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_Small_Matrix_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Gamma (const Base_Small_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_Small_Matrix_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_Small_Matrix_Data_Type* Block_00)
{
    ...
    // write the data directly
    COO_I[1] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[1];
    COO_V[1] = Block_00->FE_Tensor_1[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    ...
    COO_J_IV_Range[1] = 2;
    COO_V[2] = Block_00->FE_Tensor_2[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];

    // write (I,J,V) data for Block_00->FE_Tensor_3, i.e. the (1,1) block
    // write the data directly
    COO_I[3] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[1];
    COO_V[3] = Block_00->FE_Tensor_3[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
