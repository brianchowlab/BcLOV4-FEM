...
#define SpecificFEM        Base_Vec_Stiffness_Matrix_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Vector_P1_phi_restricted_to_Gamma->Func_f_d_ds[j][qp].a*Vector_P1_phi_restricted_to_Gamma->Func_f_d_ds[i][qp].a;
