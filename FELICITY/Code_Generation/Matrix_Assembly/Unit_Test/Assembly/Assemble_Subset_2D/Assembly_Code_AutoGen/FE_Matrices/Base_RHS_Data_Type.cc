...
// FE matrix is:
//
// \int_{Omega}  geomOmega_X2*v_v1_t1
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*(*Vh_phi_restricted_to_Omega->Func_f_Value)[i][qp].a;
