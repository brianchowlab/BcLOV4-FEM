...
public:
    ...
    void Add_Entries_To_Global_Matrix_Sigma (const Base_Vector_Mass_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Sigma(const Base_Vector_Mass_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->Vector_P1_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vector_P1_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->Vector_P1_phi_restricted_to_Sigma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vector_P1_phi_restricted_to_Sigma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    ...
    // write (I,J,V) data for Block_00->FE_Tensor_0, i.e. the (0,0) block
    // write the data directly
    COO_I[2] = Row_Indices_0[Local_Row_Ind[0]];
    COO_J[1] = Col_Indices_0[Local_Col_Ind[1]];
    COO_J_IV_Range[1] = 2;
    COO_V[2] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[0]];
    COO_I[3] = Row_Indices_0[Local_Row_Ind[1]];
    COO_V[3] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[1]];

    // write column #0
    // write (I,J,V) data for Block_00->FE_Tensor_1, i.e. the (1,1) block
    // actually:  copy Block_00->FE_Tensor_0 to Block_00->FE_Tensor_1
    // write the data directly
    COO_I[4] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[1];
    COO_J[2] = Col_Indices_0[Local_Col_Ind[0]] + Block_00->Col_Shift[1];
    COO_J_IV_Range[2] = 4;
    COO_V[4] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    COO_I[5] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[1];
    COO_V[5] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[1]];
    ...
    COO_J[3] = Col_Indices_0[Local_Col_Ind[1]] + Block_00->Col_Shift[1];
    COO_J_IV_Range[3] = 6;
    ...
    // write the data directly
    COO_I[8] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_J[4] = Col_Indices_0[Local_Col_Ind[0]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[4] = 8;
    COO_V[8] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    COO_I[9] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[2];
    COO_V[9] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[1]];

    // write column #1
    // write (I,J,V) data for Block_00->FE_Tensor_2, i.e. the (2,2) block
    // actually:  copy Block_00->FE_Tensor_0 to Block_00->FE_Tensor_2
    // write the data directly
    COO_I[10] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_J[5] = Col_Indices_0[Local_Col_Ind[1]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[5] = 10;
    COO_V[10] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[0]];
    COO_I[11] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[2];
    COO_V[11] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[1]];

    COO_J_IV_Range[6] = 12; // end of range
