    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = my_f_restricted_to_Sigma->Func_f_d_ds[0][qp].a*(*Scalar_P2_phi_restricted_to_Sigma->Func_f_Value)[j][qp].a*(*Scalar_P2_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
