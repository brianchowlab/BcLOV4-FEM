...
#define SpecificFEM        Block_Assemble_Tangent_Matrix_Data_Type
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_Tangent_Matrix_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Sigma (const Base_Tangent_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_Tangent_Matrix_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Sigma(const Base_Tangent_Matrix_Data_Type* Block_00)
{
    ...
    // write the data directly
    COO_I[2] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[1];
    COO_V[2] = Block_00->FE_Tensor_1[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    COO_I[3] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[1];
    COO_V[3] = Block_00->FE_Tensor_1[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[1]];

    // write (I,J,V) data for Block_00->FE_Tensor_2, i.e. the (2,0) block
    // write the data directly
    COO_I[4] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_V[4] = Block_00->FE_Tensor_2[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    COO_I[5] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[2];
    COO_V[5] = Block_00->FE_Tensor_2[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[1]];

    COO_J_IV_Range[1] = 6; // end of range
