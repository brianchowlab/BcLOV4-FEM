...
public:
    ...
    void Add_Entries_To_Global_Matrix_Gamma (const Base_Vector_Mass_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_Vector_Mass_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->Vector_P1_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vector_P1_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->Vector_P1_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vector_P1_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    ...
    // write the data directly
    COO_I[18] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_J[6] = Col_Indices_0[Local_Col_Ind[0]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[6] = 18;
    COO_V[18] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
    COO_I[19] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[2];
    COO_V[19] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[1]];
    COO_I[20] = Row_Indices_0[Local_Row_Ind[2]] + Block_00->Row_Shift[2];
    COO_V[20] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[2]];

    // write column #1
    // write (I,J,V) data for Block_00->FE_Tensor_2, i.e. the (2,2) block
    // actually:  copy Block_00->FE_Tensor_0 to Block_00->FE_Tensor_2
    // write the data directly
    COO_I[21] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_J[7] = Col_Indices_0[Local_Col_Ind[1]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[7] = 21;
    COO_V[21] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[0]];
    COO_I[22] = Row_Indices_0[Local_Row_Ind[1]] + Block_00->Row_Shift[2];
    COO_V[22] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[1]];
    COO_I[23] = Row_Indices_0[Local_Row_Ind[2]] + Block_00->Row_Shift[2];
    COO_V[23] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[2]];
    ...
    COO_J[8] = Col_Indices_0[Local_Col_Ind[2]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[8] = 24;
    ...
    COO_J_IV_Range[9] = 27; // end of range
