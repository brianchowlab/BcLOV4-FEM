...
#define SpecificFEM        Base_Normal_Matrix_Data_Type
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[0]*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
            const double  integrand_1 = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[1]*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
            const double  integrand_2 = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[2]*(*Vector_P1_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
