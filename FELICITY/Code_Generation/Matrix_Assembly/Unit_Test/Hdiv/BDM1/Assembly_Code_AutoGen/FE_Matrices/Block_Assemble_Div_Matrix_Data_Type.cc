...
public:
    ...
    void Add_Entries_To_Global_Matrix_Omega (const Base_Div_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Omega(const Base_Div_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->BDM1_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->BDM1_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->P0_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->P0_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
