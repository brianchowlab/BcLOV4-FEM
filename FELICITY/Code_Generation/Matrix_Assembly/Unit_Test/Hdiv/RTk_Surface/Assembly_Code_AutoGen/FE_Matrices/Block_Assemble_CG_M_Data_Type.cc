...
#define SpecificFEM        Block_Assemble_CG_M_Data_Type
#define SpecificFEM_str   "CG_M"
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_CG_M_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Gamma (const Base_CG_M_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_CG_M_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_CG_M_Data_Type* Block_00)
{
    ...
    const int col_elem_index = Block_00->V_Space_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->V_Space_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
