...
// FE matrix is:
//
// \int_{Gamma}  sig_v1_t1*tau_v1_t1 + sig_v2_t1*tau_v2_t1 + sig_v3_t1*tau_v3_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Sigma_Mass_Matrix_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[0]*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[i][qp].v[0]+Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[1]*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[i][qp].v[1]+Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[2]*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[i][qp].v[2];
