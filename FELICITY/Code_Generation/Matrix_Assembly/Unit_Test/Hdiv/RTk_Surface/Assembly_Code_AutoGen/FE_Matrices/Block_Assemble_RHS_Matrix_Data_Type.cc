...
#define SpecificFEM        Block_Assemble_RHS_Matrix_Data_Type
#define SpecificFEM_str   "RHS_Matrix"
...
// the col function space is Type = DG, Name = "lagrange_deg1_dim2"
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_RHS_Matrix_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_d_Gamma (const Base_RHS_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_RHS_Matrix_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
/*------------ BEGIN: Auto Generate ------------*/
/***************************************************************************************/
/* assemble a local FE matrix on the domain d_Gamma */
void SpecificFEM::Add_Entries_To_Global_Matrix_d_Gamma(const Base_RHS_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->Sigma_Space_phi_restricted_to_d_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Sigma_Space_phi_restricted_to_d_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->P_Space_phi_restricted_to_d_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->P_Space_phi_restricted_to_d_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    ...
    COO_J_IV_Range[3] = 24; // end of range
