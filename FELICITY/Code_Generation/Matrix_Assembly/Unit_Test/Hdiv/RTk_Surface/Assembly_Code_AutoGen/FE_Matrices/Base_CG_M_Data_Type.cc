...
// FE matrix is:
//
// \int_{Gamma}  eta_v1_t1*phi_v1_t1 + eta_v1_t2*phi_v1_t2 + eta_v1_t3*phi_v1_t3
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_CG_M_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*V_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*(*V_Space_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a;
