...
// FE matrix is:
//
// \int_{Gamma}  geomGamma_N1*sig_coef_v1_t1 + geomGamma_N2*sig_coef_v2_t1 + geomGamma_N3*sig_coef_v3_t1,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_N_DOT_sigma_Data_Type
#define SpecificFEM_str   "N_DOT_sigma"
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[0]*sig_coef_restricted_to_Gamma->Func_vv_Value[0][qp].v[0]+geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[1]*sig_coef_restricted_to_Gamma->Func_vv_Value[0][qp].v[1]+geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[2]*sig_coef_restricted_to_Gamma->Func_vv_Value[0][qp].v[2];
