...
// FE matrix is:
//
// \int_{d_Gamma}  -p_v1_t1*(tau_v1_t1*(geomGamma_N2*geomd_Gamma_T3 - geomGamma_N3*geomd_Gamma_T2) - tau_v2_t1*(geomGamma_N1*geomd_Gamma_T3 - geomGamma_N3*geomd_Gamma_T1) + tau_v3_t1*(geomGamma_N1*geomd_Gamma_T2 - geomGamma_N2*geomd_Gamma_T1))
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_RHS_Matrix_Data_Type
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor(const CLASS_geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma&);
    /*------------   END: Auto Generate ------------*/
private:
};
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma& Mesh)
{
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = -(*P_Space_phi_restricted_to_d_Gamma->Func_f_Value)[j][qp].a*(Sigma_Space_phi_restricted_to_d_Gamma->Func_vv_Value[i][qp].v[2]*(geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[0]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[1]-geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[1]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[0])-Sigma_Space_phi_restricted_to_d_Gamma->Func_vv_Value[i][qp].v[1]*(geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[0]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[2]-geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[2]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[0])+Sigma_Space_phi_restricted_to_d_Gamma->Func_vv_Value[i][qp].v[0]*(geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[1]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[2]-geom_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Normal_Vector[qp].v[2]*geom_d_Gamma_embedded_in_Gamma_restricted_to_d_Gamma->Map_Tangent_Vector[qp].v[1]));
