...
#define SpecificFEM        Block_Assemble_Div_Matrix_Data_Type
#define SpecificFEM_str   "Div_Matrix"

// the row function space is Type = DG, Name = "lagrange_deg1_dim2"
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_Div_Matrix_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Gamma (const Base_Div_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_Div_Matrix_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_Div_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->P_Space_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->P_Space_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    COO_J_IV_Range[6] = 18;
    COO_V[18] = Block_00->FE_Tensor_0[Local_Col_Ind[6]*ROW_NB + Local_Row_Ind[0]];
    COO_I[19] = Row_Indices_0[Local_Row_Ind[1]];
    COO_V[19] = Block_00->FE_Tensor_0[Local_Col_Ind[6]*ROW_NB + Local_Row_Ind[1]];
    COO_I[20] = Row_Indices_0[Local_Row_Ind[2]];
    COO_V[20] = Block_00->FE_Tensor_0[Local_Col_Ind[6]*ROW_NB + Local_Row_Ind[2]];
    ...
    COO_J_IV_Range[7] = 21;
    COO_V[21] = Block_00->FE_Tensor_0[Local_Col_Ind[7]*ROW_NB + Local_Row_Ind[0]];
    COO_I[22] = Row_Indices_0[Local_Row_Ind[1]];
    COO_V[22] = Block_00->FE_Tensor_0[Local_Col_Ind[7]*ROW_NB + Local_Row_Ind[1]];
    COO_I[23] = Row_Indices_0[Local_Row_Ind[2]];
    COO_V[23] = Block_00->FE_Tensor_0[Local_Col_Ind[7]*ROW_NB + Local_Row_Ind[2]];

    COO_J_IV_Range[8] = 24; // end of range
