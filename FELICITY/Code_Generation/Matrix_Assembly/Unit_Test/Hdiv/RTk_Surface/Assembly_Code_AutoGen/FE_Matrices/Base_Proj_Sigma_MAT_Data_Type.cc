...
// FE matrix is:
//
// \int_{Gamma}  eta_v1_t1*sig_v1_t1 + eta_v1_t2*sig_v2_t1 + eta_v1_t3*sig_v3_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Proj_Sigma_MAT_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*V_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[0];
                const double  integrand_1 = (*V_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[1];
                const double  integrand_2 = (*V_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*Sigma_Space_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[2];
            ...
            FE_Tensor_2[j*ROW_NB + i] = A2_value;
            }
        }
}
/***************************************************************************************/
