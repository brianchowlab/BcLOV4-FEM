...
#define SpecificNAME       "sig_coef"
#define SpecificFUNC        Data_Type_sig_coef_restricted_to_Gamma
#define SpecificFUNC_str   "Data_Type_sig_coef_restricted_to_Gamma"
...
public:
    ...
    const Data_Type_Sigma_Space_phi_restricted_to_Gamma*     basis_func;
    ...
    void Setup_Function_Space(const mxArray*, const Data_Type_Sigma_Space_phi_restricted_to_Gamma*);
    void Compute_Func();
private:
};
/*------------   END: Auto Generate ------------*/
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_vv_Value[nc_i][qp_i].Set_To_Zero();
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
...
void SpecificFUNC::Setup_Function_Space(const mxArray* Values,        // inputs
                                        const Data_Type_Sigma_Space_phi_restricted_to_Gamma*   basis_func_input)          // inputs
{
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            ...
            Scalar_Mult_Vector(basis_func->Func_vv_Value[0][qp_i], NodeV, Func_vv_Value[nc_i][qp_i]); // first basis function
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                NodeV.a = Node_Value[nc_i][kc[basis_i]];
                VEC_3x1  temp_vec;
                Scalar_Mult_Vector(basis_func->Func_vv_Value[basis_i][qp_i], NodeV, temp_vec);
                Add_Vector_Self(temp_vec, Func_vv_Value[nc_i][qp_i]);
                }
            }
        }
