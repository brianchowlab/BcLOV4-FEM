...
GFA::GFA (const mxArray *prhs[], const mxArray *Subset_Elem)
{
    /*------------ BEGIN: Auto Generate ------------*/
    // setup inputs
    Domain_Boundary_embedded_in_Omega_restricted_to_Boundary.Setup_Data(prhs[PRHS_Omega_Mesh_Subdomains], prhs[PRHS_Omega_Mesh_DoFmap], Subset_Elem);
    Domain_Omega_embedded_in_Omega_restricted_to_Boundary.Setup_Data(prhs[PRHS_Omega_Mesh_Subdomains], prhs[PRHS_Omega_Mesh_DoFmap], Subset_Elem);
...
void GFA::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    Base_Matrix_NV_Constraint = new Base_NV_Constraint_Data_Type(&P0_phi_restricted_to_Boundary, &RT0_phi_restricted_to_Boundary);
    Base_Matrix_NV_Matrix = new Base_NV_Matrix_Data_Type(&RT0_phi_restricted_to_Boundary, &RT0_phi_restricted_to_Boundary);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Base_Matrix_NV_Constraint->geom_Boundary_embedded_in_Omega_restricted_to_Boundary = &geom_Boundary_embedded_in_Omega_restricted_to_Boundary;
    Base_Matrix_NV_Constraint->geom_Omega_embedded_in_Omega_restricted_to_Boundary = &geom_Omega_embedded_in_Omega_restricted_to_Boundary;
    Base_Matrix_NV_Constraint->P0_phi_restricted_to_Boundary = &P0_phi_restricted_to_Boundary;
    Base_Matrix_NV_Constraint->RT0_phi_restricted_to_Boundary = &RT0_phi_restricted_to_Boundary;
    Base_Matrix_NV_Matrix->geom_Boundary_embedded_in_Omega_restricted_to_Boundary = &geom_Boundary_embedded_in_Omega_restricted_to_Boundary;
    Base_Matrix_NV_Matrix->geom_Omega_embedded_in_Omega_restricted_to_Boundary = &geom_Omega_embedded_in_Omega_restricted_to_Boundary;
    Base_Matrix_NV_Matrix->P0_phi_restricted_to_Boundary = &P0_phi_restricted_to_Boundary;
    Base_Matrix_NV_Matrix->RT0_phi_restricted_to_Boundary = &RT0_phi_restricted_to_Boundary;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the block matrices to assemble
    Block_Assemble_Matrix_NV_Constraint = new Block_Assemble_NV_Constraint_Data_Type(&Sparse_Data_NV_Constraint, Base_Matrix_NV_Constraint);
    Block_Assemble_Matrix_NV_Matrix = new Block_Assemble_NV_Matrix_Data_Type(&Sparse_Data_NV_Matrix, Base_Matrix_NV_Matrix);
    /*------------   END: Auto Generate ------------*/
}
/***************************************************************************************/
...
void GFA::Assemble_Matrices ()
{
    ...
    if (Domain_Boundary_embedded_in_Omega_restricted_to_Boundary.Sub_Assem_List.empty())
        {
        mexPrintf("This integration domain is empty: Boundary\n");
        mexPrintf(" ... so no assembly necessary.\n");
        }

    // loop through each element
    for (std::vector<unsigned int>::iterator DoI_Ind = Domain_Boundary_embedded_in_Omega_restricted_to_Boundary.Sub_Assem_List.begin();
              DoI_Ind != Domain_Boundary_embedded_in_Omega_restricted_to_Boundary.Sub_Assem_List.end(); ++DoI_Ind)
        {
        Domain_Boundary_embedded_in_Omega_restricted_to_Boundary.Read_Embed_Data(*DoI_Ind);
        Domain_Omega_embedded_in_Omega_restricted_to_Boundary.Read_Embed_Data(*DoI_Ind);
        ...
        Base_Matrix_NV_Constraint->Tabulate_Tensor(geom_Boundary_embedded_in_Omega_restricted_to_Boundary);
        Base_Matrix_NV_Matrix->Tabulate_Tensor(geom_Boundary_embedded_in_Omega_restricted_to_Boundary);

        // loop through the block FE matrices to assemble
        Block_Assemble_Matrix_NV_Constraint->Add_Entries_To_Global_Matrix_Boundary(Base_Matrix_NV_Constraint);
        Block_Assemble_Matrix_NV_Matrix->Add_Entries_To_Global_Matrix_Boundary(Base_Matrix_NV_Matrix);
        }
...
void GFA::Output_Matrices (mxArray* plhs[])
{
    ...
    Sparse_ptr = Block_Assemble_Matrix_NV_Constraint->MAT->export_matrix();
    Output_Matrix(0, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_NV_Constraint->Name), plhs[0]);
    delete(Block_Assemble_Matrix_NV_Constraint);

    Sparse_ptr = Block_Assemble_Matrix_NV_Matrix->MAT->export_matrix();
    Output_Matrix(1, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_NV_Matrix->Name), plhs[0]);
    delete(Block_Assemble_Matrix_NV_Matrix);
