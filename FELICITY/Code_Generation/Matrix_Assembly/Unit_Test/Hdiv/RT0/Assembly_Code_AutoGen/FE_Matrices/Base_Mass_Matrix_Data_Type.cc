    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = RT0_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[0]*RT0_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[0]+RT0_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[1]*RT0_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1];
