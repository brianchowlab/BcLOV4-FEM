...
GFA::GFA (const mxArray *prhs[], const mxArray *Subset_Elem)
{
    /*------------ BEGIN: Auto Generate ------------*/
    // setup inputs
    Domain_Gamma_embedded_in_Box_restricted_to_Gamma.Setup_Data(prhs[PRHS_Box_Mesh_Subdomains], prhs[PRHS_Box_Mesh_DoFmap], Subset_Elem);
    Domain_Omega_embedded_in_Box_restricted_to_Gamma.Setup_Data(prhs[PRHS_Box_Mesh_Subdomains], prhs[PRHS_Box_Mesh_DoFmap], Subset_Elem);

    geom_Gamma_embedded_in_Box_restricted_to_Gamma.Setup_Mesh_Geometry(prhs[PRHS_Box_Mesh_Vertices], prhs[PRHS_Box_Mesh_DoFmap], prhs[PRHS_Box_Mesh_Orient]);
    geom_Gamma_embedded_in_Box_restricted_to_Gamma.Domain = &Domain_Gamma_embedded_in_Box_restricted_to_Gamma;
    geom_Omega_embedded_in_Box_restricted_to_Gamma.Setup_Mesh_Geometry(prhs[PRHS_Box_Mesh_Vertices], prhs[PRHS_Box_Mesh_DoFmap], prhs[PRHS_Box_Mesh_Orient]);
    geom_Omega_embedded_in_Box_restricted_to_Gamma.Domain = &Domain_Omega_embedded_in_Box_restricted_to_Gamma;
...
void GFA::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    Access_Previous_FEM_Matrix(prhs[PRHS_OLD_FEM], "NV", 0, Sparse_Data_NV);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup access to FE basis functions
    BDM1_phi_restricted_to_Gamma.Setup_Function_Space(prhs[PRHS_BDM1_DoFmap]);
    BDM1_phi_restricted_to_Gamma.Mesh = &geom_Omega_embedded_in_Box_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup correct number of components for CONSTANT basis functions
    NV_Gamma_col_constant_phi.Num_Comp = 1;
    /*------------   END: Auto Generate ------------*/
    ...
    Base_Matrix_NV = new Base_NV_Data_Type(&BDM1_phi_restricted_to_Gamma, &NV_Gamma_col_constant_phi);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Base_Matrix_NV->geom_Gamma_embedded_in_Box_restricted_to_Gamma = &geom_Gamma_embedded_in_Box_restricted_to_Gamma;
    Base_Matrix_NV->geom_Omega_embedded_in_Box_restricted_to_Gamma = &geom_Omega_embedded_in_Box_restricted_to_Gamma;
    Base_Matrix_NV->BDM1_phi_restricted_to_Gamma = &BDM1_phi_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the block matrices to assemble
    Block_Assemble_Matrix_NV = new Block_Assemble_NV_Data_Type(&Sparse_Data_NV, Base_Matrix_NV);
    /*------------   END: Auto Generate ------------*/
}
/***************************************************************************************/
...
void GFA::Assemble_Matrices ()
{
    ...
    if (Domain_Gamma_embedded_in_Box_restricted_to_Gamma.Sub_Assem_List.empty())
        {
    ...
    for (std::vector<unsigned int>::iterator DoI_Ind = Domain_Gamma_embedded_in_Box_restricted_to_Gamma.Sub_Assem_List.begin();
              DoI_Ind != Domain_Gamma_embedded_in_Box_restricted_to_Gamma.Sub_Assem_List.end(); ++DoI_Ind)
        {
        Domain_Gamma_embedded_in_Box_restricted_to_Gamma.Read_Embed_Data(*DoI_Ind);
        Domain_Omega_embedded_in_Box_restricted_to_Gamma.Read_Embed_Data(*DoI_Ind);

        // get the local simplex transformation
        geom_Gamma_embedded_in_Box_restricted_to_Gamma.Compute_Local_Transformation();
        geom_Omega_embedded_in_Box_restricted_to_Gamma.Compute_Local_Transformation();
        ...
        Base_Matrix_NV->Tabulate_Tensor(geom_Gamma_embedded_in_Box_restricted_to_Gamma);

        // loop through the block FE matrices to assemble
        Block_Assemble_Matrix_NV->Add_Entries_To_Global_Matrix_Gamma(Base_Matrix_NV);
        }
...
void GFA::Output_Matrices (mxArray* plhs[])
{
    ...
    Sparse_ptr = Block_Assemble_Matrix_NV->MAT->export_matrix();
    Output_Matrix(0, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_NV->Name), plhs[0]);
    delete(Block_Assemble_Matrix_NV);
