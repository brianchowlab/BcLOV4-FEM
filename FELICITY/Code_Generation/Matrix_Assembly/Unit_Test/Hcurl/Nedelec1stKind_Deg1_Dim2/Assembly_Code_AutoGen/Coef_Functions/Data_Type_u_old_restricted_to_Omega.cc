...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
            Func_vv_Curl[nc_i][qp_i].a = Node_Value[nc_i][kc[0]] * basis_func->Func_vv_Curl[0][qp_i].a;
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                Func_vv_Curl[nc_i][qp_i].a += Node_Value[nc_i][kc[basis_i]] * basis_func->Func_vv_Curl[basis_i][qp_i].a;
                }
            }
        }
/*------------   END: Auto Generate ------------*/
