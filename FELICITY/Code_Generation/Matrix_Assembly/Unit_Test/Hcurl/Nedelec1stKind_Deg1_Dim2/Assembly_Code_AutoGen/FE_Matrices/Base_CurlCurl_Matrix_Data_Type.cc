...
// FE matrix is:
//
// \int_{Omega}  uu_t1_curl*vv_t1_curl
//
/*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Ned1_phi_restricted_to_Omega->Func_vv_Curl[j][qp].a*Ned1_phi_restricted_to_Omega->Func_vv_Curl[i][qp].a;
