...
// FE matrix is:
//
// \int_{Omega}  vv_v1_t1*sin(geomOmega_X1*pi) + vv_v2_t1*sin(geomOmega_X2*pi)
//
/*------------   END: Auto Generate ------------*/
...
public:
    ...
    const Data_Type_Ned1_phi_restricted_to_Omega*  Ned1_phi_restricted_to_Omega;
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = Ned1_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[0]*sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*3.141592653589793)+Ned1_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1]*sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*3.141592653589793);
