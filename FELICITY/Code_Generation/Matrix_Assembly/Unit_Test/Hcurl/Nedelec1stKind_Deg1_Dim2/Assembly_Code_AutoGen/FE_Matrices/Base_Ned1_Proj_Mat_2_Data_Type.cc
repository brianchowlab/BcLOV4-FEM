    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*P1_phi_restricted_to_Omega->Func_f_Value)[j][qp].a*Ned1_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1];
