...
// FE matrix is:
//
// \int_{Omega}  (u_soln_v1_t1 - sin(geomOmega_X1*pi))^2 + (u_soln_v2_t1 - sin(geomOmega_X2*pi))^2 + (u_soln_v3_t1 - sin(geomOmega_X3*pi))^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = pow(u_soln_restricted_to_Omega->Func_vv_Value[0][qp].v[0]-sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*3.141592653589793),2.0)+pow(u_soln_restricted_to_Omega->Func_vv_Value[0][qp].v[1]-sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*3.141592653589793),2.0)+pow(u_soln_restricted_to_Omega->Func_vv_Value[0][qp].v[2]-sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[2]*3.141592653589793),2.0);
