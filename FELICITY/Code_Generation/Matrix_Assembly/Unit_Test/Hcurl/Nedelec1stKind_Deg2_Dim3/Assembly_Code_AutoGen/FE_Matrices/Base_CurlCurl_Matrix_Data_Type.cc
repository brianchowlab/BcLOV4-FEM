...
// FE matrix is:
//
// \int_{Omega}  uu_v1_t1_curl*vv_v1_t1_curl + uu_v2_t1_curl*vv_v2_t1_curl + uu_v3_t1_curl*vv_v3_t1_curl
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_CurlCurl_Matrix_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Ned2_phi_restricted_to_Omega->Func_vv_Curl[j][qp].v[0]*Ned2_phi_restricted_to_Omega->Func_vv_Curl[i][qp].v[0]+Ned2_phi_restricted_to_Omega->Func_vv_Curl[j][qp].v[1]*Ned2_phi_restricted_to_Omega->Func_vv_Curl[i][qp].v[1]+Ned2_phi_restricted_to_Omega->Func_vv_Curl[j][qp].v[2]*Ned2_phi_restricted_to_Omega->Func_vv_Curl[i][qp].v[2];
