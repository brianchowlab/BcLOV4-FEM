...
// FE matrix is:
//
// \int_{Omega}  uu_v1_t1*vv_v1_t1 + uu_v2_t1*vv_v2_t1 + uu_v3_t1*vv_v3_t1
//
/*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Ned2_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[0]*Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[0]+Ned2_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[1]*Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1]+Ned2_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[2]*Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[2];
