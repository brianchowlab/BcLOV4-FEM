...
// FE matrix is:
//
// \int_{Omega}  u_old_v1_t1_curl + u_old_v2_t1_curl + u_old_v3_t1_curl,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = u_old_restricted_to_Omega->Func_vv_Curl[0][qp].v[0]+u_old_restricted_to_Omega->Func_vv_Curl[0][qp].v[1]+u_old_restricted_to_Omega->Func_vv_Curl[0][qp].v[2];
