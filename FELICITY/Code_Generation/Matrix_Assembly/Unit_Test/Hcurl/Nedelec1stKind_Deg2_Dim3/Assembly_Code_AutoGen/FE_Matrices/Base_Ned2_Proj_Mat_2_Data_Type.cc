...
// FE matrix is:
//
// \int_{Omega}  vv_v1_t1*exp(-geomOmega_X2) + geomOmega_X1^2*vv_v2_t1 + geomOmega_X3^3*vv_v3_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Ned2_Proj_Mat_2_Data_Type
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = (geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0])*Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1]+(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[2]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[2]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[2])*Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[2]+Ned2_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[0]*exp(-geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]);
