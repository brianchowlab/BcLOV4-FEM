...
#define SpecificNAME       "u_old"
#define SpecificFUNC        Data_Type_u_old_restricted_to_Omega
#define SpecificFUNC_str   "Data_Type_u_old_restricted_to_Omega"
...
public:
    ...
    const Data_Type_Ned2_phi_restricted_to_Omega*     basis_func;
    ...
    // curl of coefficient function evaluated at a quadrature point in reference element
    VEC_3x1 Func_vv_Curl[NC][NQ];

    // constructor
    SpecificFUNC ();
    ~SpecificFUNC (); // destructor
    void Setup_Function_Space(const mxArray*, const Data_Type_Ned2_phi_restricted_to_Omega*);
    void Compute_Func();
private:
};
/*------------   END: Auto Generate ------------*/
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_vv_Curl[nc_i][qp_i].Set_To_Zero();
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFUNC::Setup_Function_Space(const mxArray* Values,        // inputs
                                        const Data_Type_Ned2_phi_restricted_to_Omega*   basis_func_input)          // inputs
{
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    // get coefficient (curl) function values
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
            SCALAR NodeV;
            NodeV.a = Node_Value[nc_i][kc[0]];
            Scalar_Mult_Vector(basis_func->Func_vv_Curl[0][qp_i], NodeV, Func_vv_Curl[nc_i][qp_i]); // first basis function
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                NodeV.a = Node_Value[nc_i][kc[basis_i]];
                VEC_3x1  temp_vec;
                Scalar_Mult_Vector(basis_func->Func_vv_Curl[basis_i][qp_i], NodeV, temp_vec);
                Add_Vector_Self(temp_vec, Func_vv_Curl[nc_i][qp_i]);
                }
            }
        }
/*------------   END: Auto Generate ------------*/
