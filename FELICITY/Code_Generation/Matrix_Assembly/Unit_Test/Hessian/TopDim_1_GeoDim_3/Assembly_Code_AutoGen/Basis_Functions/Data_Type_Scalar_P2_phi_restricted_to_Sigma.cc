...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            ...
            // compute dot product of tangent vector with Hess(PHI)
            const double DP_TV_Hess = Mesh->Map_PHI_Hess[qp_i].m[0][0][0] * Mesh->Map_Tangent_Vector[qp_i].v[0] + Mesh->Map_PHI_Hess[qp_i].m[1][0][0] * Mesh->Map_Tangent_Vector[qp_i].v[1] + Mesh->Map_PHI_Hess[qp_i].m[2][0][0] * Mesh->Map_Tangent_Vector[qp_i].v[2];
            const double Diff_Hess_Terms = phi_d2_ds2[qp_i][basis_i].a - ( Func_f_d_ds[basis_i][qp_i].a * DP_TV_Hess );
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            MAT_3x3  tan_otimes_tan; // compute tensor product of tangent vector
            Outer_Product_Vec(Mesh->Map_Tangent_Vector[qp_i], Mesh->Map_Tangent_Vector[qp_i], tan_otimes_tan);
            // set the hessian matrix with [vt x vt] d^2/ds^2, where vt is the tangent vector
            Scalar_Mult_Matrix(tan_otimes_tan, Func_f_d2_ds2[basis_i][qp_i], Func_f_Hess[basis_i][qp_i]);
            // map is non-linear, so substract [vk x vt] d/ds, where vk is the curvature vector
            MAT_3x3  curv_otimes_tan; // compute tensor product of tangent vector with curvature vector
            Outer_Product_Vec(Mesh->Map_Total_Curvature_Vector[qp_i], Mesh->Map_Tangent_Vector[qp_i], curv_otimes_tan);
            MAT_3x3  non_symm_term; // temp variable
            Scalar_Mult_Matrix(curv_otimes_tan, Func_f_d_ds[basis_i][qp_i], non_symm_term);
            // compute difference: [vt x vt] d^2/ds^2 - [vk x vt] d/ds
            Subtract_Matrix(Func_f_Hess[basis_i][qp_i], non_symm_term, Func_f_Hess[basis_i][qp_i]);
            }
        }
/*------------   END: Auto Generate ------------*/
}
