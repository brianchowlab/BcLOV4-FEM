...
#define SpecificFUNC        Data_Type_Vector_P2_phi_restricted_to_Gamma
#define SpecificFUNC_str   "Data_Type_Vector_P2_phi_restricted_to_Gamma"
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            MAT_2x2  temp_hess_MAT(phi_Hess[qp_i][basis_i]); // init to hess(phi)
            // compute the Christoffel correction
            MAT_2x2  Christoffel_Gamma_k; // \Gamma^k_{i,j}, k = 0, 1
            Mesh->Map_PHI_Christoffel_2nd_Kind[qp_i].Extract_Comp_Matrix(0, Christoffel_Gamma_k);
            // multiply by basis function derivative
            Scalar_Mult_Matrix (Christoffel_Gamma_k, phi_Grad[qp_i][basis_i].v[0], Christoffel_Gamma_k);
            Subtract_Matrix(temp_hess_MAT, Christoffel_Gamma_k, temp_hess_MAT);
            Mesh->Map_PHI_Christoffel_2nd_Kind[qp_i].Extract_Comp_Matrix(1, Christoffel_Gamma_k);
            // multiply by basis function derivative
            Scalar_Mult_Matrix (Christoffel_Gamma_k, phi_Grad[qp_i][basis_i].v[1], Christoffel_Gamma_k);
            Subtract_Matrix(temp_hess_MAT, Christoffel_Gamma_k, temp_hess_MAT);
            MAT_3x2  sub_MAT, J_g_inv_MAT;
            // multiply: (Grad(PHI)) * g^{-1}
            Mat_Mat(Mesh->Map_PHI_Grad[qp_i], Mesh->Map_PHI_Inv_Metric[qp_i], J_g_inv_MAT);
            // multiply: [(Grad(PHI)) * g^{-1}] [local hess(f)] [(Grad(PHI)) * g^{-1}]^T
            Mat_Mat(J_g_inv_MAT, temp_hess_MAT, sub_MAT);
            Mat_Mat_Transpose(sub_MAT, J_g_inv_MAT, Func_f_Hess[basis_i][qp_i]);
            }
        }
/*------------   END: Auto Generate ------------*/
}
