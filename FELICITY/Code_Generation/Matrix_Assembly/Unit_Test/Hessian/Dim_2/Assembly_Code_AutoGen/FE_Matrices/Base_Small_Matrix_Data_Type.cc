...
// FE matrix is:
//
// \int_{Omega}  Vec_v1_t2_hess11 + Vec_v1_t2_hess22,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  integrand_0 = Vec_restricted_to_Omega->Func_f_Hess[1][qp].m[0][0]+Vec_restricted_to_Omega->Func_f_Hess[1][qp].m[1][1];
