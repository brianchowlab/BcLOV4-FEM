...
// FE matrix is:
//
// \int_{Sigma}  u_v1_t1_dsds*v_v1_t1_dsds
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_dsds_Matrix_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Scalar_P2_phi_restricted_to_Sigma->Func_f_d2_ds2[j][qp].a*Scalar_P2_phi_restricted_to_Sigma->Func_f_d2_ds2[i][qp].a;
