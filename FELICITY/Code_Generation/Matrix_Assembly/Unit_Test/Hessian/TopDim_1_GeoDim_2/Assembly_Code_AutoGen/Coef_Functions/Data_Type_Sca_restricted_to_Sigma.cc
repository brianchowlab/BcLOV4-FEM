...
#define SpecificNAME       "Sca"
#define SpecificFUNC        Data_Type_Sca_restricted_to_Sigma
#define SpecificFUNC_str   "Data_Type_Sca_restricted_to_Sigma"
...
public:
    ...
    // 2nd arc-length derivative of coefficient function
    SCALAR Func_f_d2_ds2[NC][NQ];
    // (intrinsic) hessian of coefficient function
    MAT_2x2 Func_f_Hess[NC][NQ];
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_f_d2_ds2[nc_i][qp_i].Set_To_Zero();
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Func_f_Hess[nc_i][qp_i].Set_To_Zero();
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    // get 2nd arc-length derivative of coefficient function
    // loop through quad points
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        {
        // loop through all components (indexing is in the C style)
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
            Func_f_d2_ds2[nc_i][qp_i].a = basis_func->Func_f_d2_ds2[0][qp_i].a * Node_Value[nc_i][kc[0]]; // first basis function
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                Func_f_d2_ds2[nc_i][qp_i].a += basis_func->Func_f_d2_ds2[basis_i][qp_i].a * Node_Value[nc_i][kc[basis_i]];
                }
            }
        }
    // get hessian of coefficient function
    // loop through quad points
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        {
        // loop through all components (indexing is in the C style)
        for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
            {
        Scalar_Mult_Matrix(basis_func->Func_f_Hess[0][qp_i], Node_Value[nc_i][kc[0]], Func_f_Hess[nc_i][qp_i]);
            // sum over basis functions
            for (int basis_i = 1; (basis_i < NB); basis_i++)
                {
                MAT_2x2  Temp_MAT;
                Scalar_Mult_Matrix(basis_func->Func_f_Hess[basis_i][qp_i], Node_Value[nc_i][kc[basis_i]], Temp_MAT);
                Add_Matrix(Func_f_Hess[nc_i][qp_i], Temp_MAT, Func_f_Hess[nc_i][qp_i]);
                }
            }
        }
/*------------   END: Auto Generate ------------*/
