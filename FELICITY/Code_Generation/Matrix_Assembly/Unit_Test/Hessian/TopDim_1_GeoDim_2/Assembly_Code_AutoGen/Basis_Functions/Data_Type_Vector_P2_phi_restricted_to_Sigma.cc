...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            ...
            const double DP_TV_Hess = Mesh->Map_PHI_Hess[qp_i].m[0][0][0] * Mesh->Map_Tangent_Vector[qp_i].v[0] + Mesh->Map_PHI_Hess[qp_i].m[1][0][0] * Mesh->Map_Tangent_Vector[qp_i].v[1];
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            MAT_2x2  tan_otimes_tan; // compute tensor product of tangent vector
            ...
            MAT_2x2  curv_otimes_tan; // compute tensor product of tangent vector with curvature vector
            Outer_Product_Vec(Mesh->Map_Total_Curvature_Vector[qp_i], Mesh->Map_Tangent_Vector[qp_i], curv_otimes_tan);
            MAT_2x2  non_symm_term; // temp variable
