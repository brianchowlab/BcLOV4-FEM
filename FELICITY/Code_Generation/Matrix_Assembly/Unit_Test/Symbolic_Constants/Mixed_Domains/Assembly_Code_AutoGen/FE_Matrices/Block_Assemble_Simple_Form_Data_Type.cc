...
#define SpecificFEM        Block_Assemble_Simple_Form_Data_Type
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_Simple_Form_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Gamma (const Base_Simple_Form_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_Simple_Form_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_Simple_Form_Data_Type* Block_00)
{
    ...
    COO_J_IV_Range[1] = 1;
    COO_V[1] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];

    // write column #0
    // write (I,J,V) data for Block_00->FE_Tensor_2, i.e. the (2,2) block
    // actually:  copy Block_00->FE_Tensor_0 to Block_00->FE_Tensor_2
    // write the data directly
    COO_I[2] = Row_Indices_0[Local_Row_Ind[0]] + Block_00->Row_Shift[2];
    COO_J[2] = Col_Indices_0[Local_Col_Ind[0]] + Block_00->Col_Shift[2];
    COO_J_IV_Range[2] = 2;
    COO_V[2] = Block_00->FE_Tensor_0[Local_Col_Ind[0]*ROW_NB + Local_Row_Ind[0]];
