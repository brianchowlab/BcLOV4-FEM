...
{
    // clear it
    mxFree(Sparse_Data_Eval_Const.name);
...
void GFA::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    Access_Previous_FEM_Matrix(prhs[PRHS_OLD_FEM], "Eval_Const", 0, Sparse_Data_Eval_Const);
    Access_Previous_FEM_Matrix(prhs[PRHS_OLD_FEM], "Simple_Form", 1, Sparse_Data_Simple_Form);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup access to FE basis functions
    Const_Space_phi_restricted_to_Gamma.Setup_Function_Space(Const_Space_phi_restricted_to_Gamma.CONST_DoFmap);
    Const_Space_phi_restricted_to_Gamma.Mesh = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma.Setup_Function_Space(Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma.CONST_DoFmap);
    Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma.Mesh = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup correct number of components for CONSTANT basis functions
    Eval_Const_Gamma_col_constant_phi.Num_Comp = 1;
    Eval_Const_Gamma_row_constant_phi.Num_Comp = 1;
    Simple_Form_Gamma_col_constant_phi.Num_Comp = 1;
    Simple_Form_Gamma_row_constant_phi.Num_Comp = 1;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup access to external FE functions
    c0_restricted_to_Gamma.Setup_Function_Space(prhs[PRHS_c0_Values], &Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma);
    // read in the external constant data (once!)
    for (int nc_i = 0; (nc_i < c0_restricted_to_Gamma.Num_Comp); nc_i++)
        c0_restricted_to_Gamma.Constant_C_Value[nc_i].a = c0_restricted_to_Gamma.Node_Value[nc_i][0];
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the base matrices to compute
    Base_Matrix_Eval_Const = new Base_Eval_Const_Data_Type(&Eval_Const_Gamma_row_constant_phi, &Eval_Const_Gamma_col_constant_phi);
    Base_Matrix_Simple_Form = new Base_Simple_Form_Data_Type(&Const_Space_phi_restricted_to_Gamma, &Const_Space_phi_restricted_to_Gamma);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Base_Matrix_Eval_Const->geom_Gamma_embedded_in_Omega_restricted_to_Gamma = &geom_Gamma_embedded_in_Omega_restricted_to_Gamma;
    Base_Matrix_Eval_Const->geom_Omega_embedded_in_Omega_restricted_to_Gamma = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    Base_Matrix_Eval_Const->Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma = &Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma;
    Base_Matrix_Eval_Const->Const_Space_phi_restricted_to_Gamma = &Const_Space_phi_restricted_to_Gamma;
    Base_Matrix_Eval_Const->c0_restricted_to_Gamma = &c0_restricted_to_Gamma;
    Base_Matrix_Simple_Form->geom_Gamma_embedded_in_Omega_restricted_to_Gamma = &geom_Gamma_embedded_in_Omega_restricted_to_Gamma;
    Base_Matrix_Simple_Form->geom_Omega_embedded_in_Omega_restricted_to_Gamma = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    Base_Matrix_Simple_Form->Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma = &Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma;
    Base_Matrix_Simple_Form->Const_Space_phi_restricted_to_Gamma = &Const_Space_phi_restricted_to_Gamma;
    Base_Matrix_Simple_Form->c0_restricted_to_Gamma = &c0_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the block matrices to assemble
    Block_Assemble_Matrix_Eval_Const = new Block_Assemble_Eval_Const_Data_Type(&Sparse_Data_Eval_Const, Base_Matrix_Eval_Const);
...
void GFA::Assemble_Matrices ()
{
    ...
    for (std::vector<unsigned int>::iterator DoI_Ind = Domain_Gamma_embedded_in_Omega_restricted_to_Gamma.Sub_Assem_List.begin();
        ...
        Const_Space_phi_restricted_to_Gamma.Transform_Basis_Functions();
        Const_Space_Omega_TupleSize_3_1_phi_restricted_to_Gamma.Transform_Basis_Functions();

        // perform pre-computations with external FE coefficient functions
        c0_restricted_to_Gamma.Compute_Func();

        // loop through the FE matrices to compute
        Base_Matrix_Eval_Const->Tabulate_Tensor(geom_Gamma_embedded_in_Omega_restricted_to_Gamma);
        Base_Matrix_Simple_Form->Tabulate_Tensor(geom_Gamma_embedded_in_Omega_restricted_to_Gamma);

        // loop through the block FE matrices to assemble
        Block_Assemble_Matrix_Eval_Const->Add_Entries_To_Global_Matrix_Gamma(Base_Matrix_Eval_Const);
        Block_Assemble_Matrix_Simple_Form->Add_Entries_To_Global_Matrix_Gamma(Base_Matrix_Simple_Form);
        }
...
void GFA::Output_Matrices (mxArray* plhs[])
{
    ...
    Sparse_ptr = Block_Assemble_Matrix_Eval_Const->MAT->export_matrix();
    Output_Matrix(0, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_Eval_Const->Name), plhs[0]);
    delete(Block_Assemble_Matrix_Eval_Const);

    Sparse_ptr = Block_Assemble_Matrix_Simple_Form->MAT->export_matrix();
    Output_Matrix(1, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_Simple_Form->Name), plhs[0]);
