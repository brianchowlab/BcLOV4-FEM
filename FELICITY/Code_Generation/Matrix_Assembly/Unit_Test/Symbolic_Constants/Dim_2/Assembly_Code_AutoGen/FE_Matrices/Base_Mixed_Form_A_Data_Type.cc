...
// FE matrix is:
//
// \int_{Omega}  b_v1_t1*v_v1_t1 + b_v1_t2*v_v1_t2
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Mixed_Form_A_Data_Type
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = Const_Space_phi_restricted_to_Omega->Constant_C_Value.a*(*Vector_P1_phi_restricted_to_Omega->Func_f_Value)[i][qp].a;
