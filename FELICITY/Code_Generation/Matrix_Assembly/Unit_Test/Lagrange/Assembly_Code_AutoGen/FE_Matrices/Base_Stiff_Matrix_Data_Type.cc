    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = V_h_phi_restricted_to_Omega->Func_f_Grad[j][qp].v[0]*V_h_phi_restricted_to_Omega->Func_f_Grad[i][qp].v[0]+V_h_phi_restricted_to_Omega->Func_f_Grad[j][qp].v[1]*V_h_phi_restricted_to_Omega->Func_f_Grad[i][qp].v[1];
