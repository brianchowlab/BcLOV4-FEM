...
// FE matrix is:
//
// \int_{Omega}  (discrete_soln_v1_t1_grad2 - geomOmega_X1*geomOmega_X2*cos(geomOmega_X2)*sin(geomOmega_X1)*(geomOmega_X1 - 1) - geomOmega_X1*cos(geomOmega_X2)*sin(geomOmega_X1)*(geomOmega_X1 - 1)*(geomOmega_X2 - 1) + geomOmega_X1*geomOmega_X2*sin(geomOmega_X1)*sin(geomOmega_X2)*(geomOmega_X1 - 1)*(geomOmega_X2 - 1))^2 + (geomOmega_X1*geomOmega_X2*cos(geomOmega_X2)*sin(geomOmega_X1)*(geomOmega_X2 - 1) - discrete_soln_v1_t1_grad1 + geomOmega_X2*cos(geomOmega_X2)*sin(geomOmega_X1)*(geomOmega_X1 - 1)*(geomOmega_X2 - 1) + geomOmega_X1*geomOmega_X2*cos(geomOmega_X1)*cos(geomOmega_X2)*(geomOmega_X1 - 1)*(geomOmega_X2 - 1))^2,  ... etc... (see individual submatrices, i.e. Tab_Tensor routines.)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_L2_Grad_Error_Sq_Data_Type
#define SpecificFEM_str   "L2_Grad_Error_Sq"
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_embedded_in_Omega_restricted_to_Omega& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  t2 = cos(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]);
        const double  t3 = sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]);
        const double  t4 = geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]-1.0;
        const double  t5 = geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]-1.0;
        const double  integrand_0 = pow(-discrete_soln_restricted_to_Omega->Func_f_Grad[0][qp].v[0]+geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*t2*t3*t5+geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*t2*t3*t4*t5+geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*t2*t4*t5*cos(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]),2.0)+pow(discrete_soln_restricted_to_Omega->Func_f_Grad[0][qp].v[1]-geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*t2*t3*t4-geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*t2*t3*t4*t5+geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[0]*geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]*t3*t4*t5*sin(geom_Omega_embedded_in_Omega_restricted_to_Omega->Map_PHI[qp].v[1]),2.0);
