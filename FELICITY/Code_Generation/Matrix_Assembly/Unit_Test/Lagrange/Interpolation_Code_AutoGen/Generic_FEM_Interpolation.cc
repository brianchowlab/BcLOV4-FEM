...
void GFI::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    u0_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_u0_Values], &V_h_phi_restricted_to_Omega);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the desired interpolations to evaluate
    Iobj_I_u0 = new I_u0(Omega_Interp_Data.Num_Pts);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Iobj_I_u0->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_u0->u0_restricted_to_Omega = &u0_restricted_to_Omega;
...
void GFI::Evaluate_Interpolations ()
{
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            // copy local interpolation coordinates
            Omega_Interp_Data.Copy_Local_X(DoE_Pt,V_h_phi_restricted_to_Omega.local_coord);
            ...
            Iobj_I_u0->Eval_All_Interpolations(DoE_Pt);
            }
        }
...
{
    Output_Single_Interpolation(0, Iobj_I_u0->mxInterp_Data, mxCreateString(Iobj_I_u0->Name), plhs[0]);
    delete(Iobj_I_u0);
