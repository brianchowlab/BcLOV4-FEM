...
GFA::GFA (const mxArray *prhs[], const mxArray *Subset_Elem)
{
    /*------------ BEGIN: Auto Generate ------------*/
    // setup inputs
    Domain_Gamma_embedded_in_Gamma_restricted_to_Sigma.Setup_Data(prhs[PRHS_Gamma_Mesh_Subdomains], prhs[PRHS_Gamma_Mesh_DoFmap], Subset_Elem);
    Domain_Sigma_embedded_in_Gamma_restricted_to_Sigma.Setup_Data(prhs[PRHS_Gamma_Mesh_Subdomains], prhs[PRHS_Gamma_Mesh_DoFmap], Subset_Elem);
...
void GFA::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    Access_Previous_FEM_Matrix(prhs[PRHS_OLD_FEM], "Chi", 0, Sparse_Data_Chi);
    ...
    Chi_Sigma_col_constant_phi.Num_Comp = 1;
    /*------------   END: Auto Generate ------------*/
    ...
    Base_Matrix_Chi = new Base_Chi_Data_Type(&V_Space_phi_restricted_to_Sigma, &Chi_Sigma_col_constant_phi);
    Base_Matrix_Mass_Matrix = new Base_Mass_Matrix_Data_Type(&V_Space_phi_restricted_to_Sigma, &V_Space_phi_restricted_to_Sigma);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Base_Matrix_Chi->geom_Gamma_embedded_in_Gamma_restricted_to_Sigma = &geom_Gamma_embedded_in_Gamma_restricted_to_Sigma;
    Base_Matrix_Chi->geom_Sigma_embedded_in_Gamma_restricted_to_Sigma = &geom_Sigma_embedded_in_Gamma_restricted_to_Sigma;
    Base_Matrix_Chi->V_Space_phi_restricted_to_Sigma = &V_Space_phi_restricted_to_Sigma;
    Base_Matrix_Mass_Matrix->geom_Gamma_embedded_in_Gamma_restricted_to_Sigma = &geom_Gamma_embedded_in_Gamma_restricted_to_Sigma;
    Base_Matrix_Mass_Matrix->geom_Sigma_embedded_in_Gamma_restricted_to_Sigma = &geom_Sigma_embedded_in_Gamma_restricted_to_Sigma;
    Base_Matrix_Mass_Matrix->V_Space_phi_restricted_to_Sigma = &V_Space_phi_restricted_to_Sigma;
    /*------------   END: Auto Generate ------------*/
...
void GFA::Assemble_Matrices ()
{
    ...
    if (Domain_Sigma_embedded_in_Gamma_restricted_to_Sigma.Sub_Assem_List.empty())
        {
    ...
    for (std::vector<unsigned int>::iterator DoI_Ind = Domain_Sigma_embedded_in_Gamma_restricted_to_Sigma.Sub_Assem_List.begin();
              DoI_Ind != Domain_Sigma_embedded_in_Gamma_restricted_to_Sigma.Sub_Assem_List.end(); ++DoI_Ind)
        {
        Domain_Gamma_embedded_in_Gamma_restricted_to_Sigma.Read_Embed_Data(*DoI_Ind);
        Domain_Sigma_embedded_in_Gamma_restricted_to_Sigma.Read_Embed_Data(*DoI_Ind);
        ...
        Base_Matrix_Chi->Tabulate_Tensor(geom_Sigma_embedded_in_Gamma_restricted_to_Sigma);
        Base_Matrix_Mass_Matrix->Tabulate_Tensor(geom_Sigma_embedded_in_Gamma_restricted_to_Sigma);

        // loop through the block FE matrices to assemble
        Block_Assemble_Matrix_Chi->Add_Entries_To_Global_Matrix_Sigma(Base_Matrix_Chi);
...
void GFA::Output_Matrices (mxArray* plhs[])
{
    ...
    Output_Matrix(0, Sparse_ptr, mxCreateString(Block_Assemble_Matrix_Chi->Name), plhs[0]);
