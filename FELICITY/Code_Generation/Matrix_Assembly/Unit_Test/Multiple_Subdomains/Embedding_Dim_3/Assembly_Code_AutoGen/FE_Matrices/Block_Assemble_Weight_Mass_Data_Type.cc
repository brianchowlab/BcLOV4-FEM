...
#define SpecificFEM        Block_Assemble_Weight_Mass_Data_Type
#define SpecificFEM_str   "Weight_Mass"
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_Weight_Mass_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Omega_1 (const Base_Weight_Mass_Data_Type*);
    void Add_Entries_To_Global_Matrix_Omega_2 (const Base_Weight_Mass_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_Weight_Mass_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
/*------------ BEGIN: Auto Generate ------------*/
/***************************************************************************************/
/* assemble a local FE matrix on the domain Omega_1 */
void SpecificFEM::Add_Entries_To_Global_Matrix_Omega_1(const Base_Weight_Mass_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->U_Space_phi_restricted_to_Omega_1->Mesh->Domain->Sub_Cell_Index;
    Block_00->U_Space_phi_restricted_to_Omega_1->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->U_Space_phi_restricted_to_Omega_1->Mesh->Domain->Sub_Cell_Index;
    Block_00->U_Space_phi_restricted_to_Omega_1->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    ...
    COO_J_IV_Range[4] = 16; // end of range
...
/*------------ BEGIN: Auto Generate ------------*/
/***************************************************************************************/
/* assemble a local FE matrix on the domain Omega_2 */
void SpecificFEM::Add_Entries_To_Global_Matrix_Omega_2(const Base_Weight_Mass_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->U_Space_phi_restricted_to_Omega_2->Mesh->Domain->Sub_Cell_Index;
    Block_00->U_Space_phi_restricted_to_Omega_2->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    // shift Row_Indices_0 to account for the block matrix offset
    for (unsigned int ri = 0; ri < ROW_NB; ++ri)
        Row_Indices_0[ri] += Block_Row_Shift[0];
    ...
    // get local to global index map for the current COL element
    int  Col_Indices_0[COL_NB];
    const int col_elem_index = Block_00->U_Space_phi_restricted_to_Omega_2->Mesh->Domain->Sub_Cell_Index;
    Block_00->U_Space_phi_restricted_to_Omega_2->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    // shift Col_Indices_0 to account for the block matrix offset
    for (unsigned int ci = 0; ci < COL_NB; ++ci)
        Col_Indices_0[ci] += Block_Col_Shift[0];
