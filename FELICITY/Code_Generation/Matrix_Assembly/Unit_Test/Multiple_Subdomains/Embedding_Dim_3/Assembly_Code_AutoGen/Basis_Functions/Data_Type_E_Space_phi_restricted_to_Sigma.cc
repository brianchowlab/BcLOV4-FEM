...
#define SpecificFUNC        Data_Type_E_Space_phi_restricted_to_Sigma
#define SpecificFUNC_str   "Data_Type_E_Space_phi_restricted_to_Sigma"
...
public:
    ...
    const CLASS_geom_Sigma_embedded_in_Global_restricted_to_Sigma*  Mesh;
...
// Number of Quadrature Points = 3
    ...
    phi_Grad[2][0].Set_Equal_To(-1.00000000000000000E+00);
    phi_Grad[2][1].Set_Equal_To(1.00000000000000000E+00);
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            Scalar_Mult_Vector(Mesh->Map_Tangent_Vector[0], Func_f_d_ds[basis_i][qp_i], Func_f_Grad[basis_i][qp_i]);
            }
        }
/*------------   END: Auto Generate ------------*/
}
