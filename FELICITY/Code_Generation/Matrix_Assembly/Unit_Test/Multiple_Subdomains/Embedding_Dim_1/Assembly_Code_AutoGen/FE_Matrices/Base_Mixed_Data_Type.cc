...
// FE matrix is:
//
// \int_{Omega_1}  lambda_v1_t1*v_v1_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Mixed_Data_Type
#define SpecificFEM_str   "Mixed"
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*M_Space_phi_restricted_to_Omega_1->Func_f_Value)[j][qp].a*(*U_Space_phi_restricted_to_Omega_1->Func_f_Value)[i][qp].a;
