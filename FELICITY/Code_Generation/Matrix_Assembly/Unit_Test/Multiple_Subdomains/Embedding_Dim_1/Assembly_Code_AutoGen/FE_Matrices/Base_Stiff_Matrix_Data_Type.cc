...
// FE matrix is:
//
// \int_{Omega_1}  u_v1_t1_grad1*v_v1_t1_grad1 + 
// \int_{Omega_2}  10*u_v1_t1_grad1*v_v1_t1_grad1
//
/*------------   END: Auto Generate ------------*/
...
public:
    ...
    const Data_Type_M_Space_phi_restricted_to_Omega_1*  M_Space_phi_restricted_to_Omega_1;
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = U_Space_phi_restricted_to_Omega_1->Func_f_Grad[j][qp].v[0]*U_Space_phi_restricted_to_Omega_1->Func_f_Grad[i][qp].v[0];
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Omega_2_embedded_in_Global_restricted_to_Omega_2& Mesh)
{
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = U_Space_phi_restricted_to_Omega_2->Func_f_Grad[j][qp].v[0]*U_Space_phi_restricted_to_Omega_2->Func_f_Grad[i][qp].v[0]*1.0E+1;
                A0_value += integrand_0 * Mesh.Map_Det_Jac_w_Weight[qp].a;
                }
            FE_Tensor_0[j*ROW_NB + i] = A0_value;
            }
