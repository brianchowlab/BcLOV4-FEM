...
// FE matrix is:
//
// \int_{Gamma}  lambda_v1_t1*v1_v1_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Bdy_1_Data_Type
#define SpecificFEM_str   "Bdy_1"
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*M_Space_phi_restricted_to_Gamma->Func_f_Value)[j][qp].a*(*V1_Space_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a;
