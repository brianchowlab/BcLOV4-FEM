...
// FE matrix is:
//
// \int_{Gamma}  lambda_v1_t1_ds*old_soln_v1_t1*(geomGamma_N1*v_v1_t1_grad1 + geomGamma_N2*v_v1_t1_grad2)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_Bdy_Geo_Data_Type
#define SpecificFEM_str   "Bdy_Geo"
...
public:
    ...
    const CLASS_geom_Omega_1_embedded_in_Global_restricted_to_Gamma*  geom_Omega_1_embedded_in_Global_restricted_to_Gamma;
    const CLASS_geom_Omega_2_embedded_in_Global_restricted_to_Gamma*  geom_Omega_2_embedded_in_Global_restricted_to_Gamma;
    ...
    const Data_Type_V1_Space_phi_restricted_to_Gamma*  V1_Space_phi_restricted_to_Gamma;
    const Data_Type_V2_Space_phi_restricted_to_Gamma*  V2_Space_phi_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = M_Space_phi_restricted_to_Gamma->Func_f_d_ds[j][qp].a*old_soln_restricted_to_Gamma->Func_f_Value[0][qp].a*(geom_Gamma_embedded_in_Global_restricted_to_Gamma->Map_Normal_Vector[0].v[0]*U_Space_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[0]+geom_Gamma_embedded_in_Global_restricted_to_Gamma->Map_Normal_Vector[0].v[1]*U_Space_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[1]);
