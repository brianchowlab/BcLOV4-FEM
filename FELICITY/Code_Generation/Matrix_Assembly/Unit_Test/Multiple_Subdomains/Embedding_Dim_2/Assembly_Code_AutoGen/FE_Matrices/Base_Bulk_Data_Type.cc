...
#define SpecificFEM        Base_Bulk_Data_Type
#define SpecificFEM_str   "Bulk"
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*U_Space_phi_restricted_to_Omega->Func_f_Value)[j][qp].a*(*U_Space_phi_restricted_to_Omega->Func_f_Value)[i][qp].a;
