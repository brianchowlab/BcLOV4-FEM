...
//
//    matrix([[geomSigma_T1], [geomSigma_T2], [geomSigma_T3]]), on Sigma.
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM interpolation (should be the same as the filename of this file)
#define SpecificFEM        I_tangent
#define SpecificFEM_str   "I_tangent"
...
void SpecificFEM::Eval_Interp_00(double& INTERP)
{
    ...
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Tangent_Vector[qp].v[0];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_10(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Tangent_Vector[qp].v[1];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_20(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Tangent_Vector[qp].v[2];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
