...
//
//    matrix([[geomGamma_N1], [geomGamma_N2], [geomGamma_N3]]), on Gamma.
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM interpolation (should be the same as the filename of this file)
#define SpecificFEM        I_normal
#define SpecificFEM_str   "I_normal"
...
void SpecificFEM::Eval_Interp_00(double& INTERP)
{
    ...
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[0];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_10(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[1];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_20(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_Normal_Vector[qp].v[2];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
