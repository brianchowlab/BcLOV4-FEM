...
void GFI::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    Scalar_P2_phi_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_Scalar_P2_DoFmap]);
    Scalar_P2_phi_restricted_to_Omega.Mesh = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    ...
    p_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_p_Values], &Scalar_P2_phi_restricted_to_Omega);
    v_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_v_Values], &Vector_P1_phi_restricted_to_Omega);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the desired interpolations to evaluate
    Iobj_I_p = new I_p(Omega_Interp_Data.Num_Pts);
    Iobj_I_v = new I_v(Omega_Interp_Data.Num_Pts);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Iobj_I_p->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_p->p_restricted_to_Omega = &p_restricted_to_Omega;
    Iobj_I_p->v_restricted_to_Omega = &v_restricted_to_Omega;

    Iobj_I_v->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_v->p_restricted_to_Omega = &p_restricted_to_Omega;
...
void GFI::Evaluate_Interpolations ()
{
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            // copy local interpolation coordinates
            Omega_Interp_Data.Copy_Local_X(DoE_Pt,Scalar_P2_phi_restricted_to_Omega.local_coord);
            Scalar_P2_phi_restricted_to_Omega.Transform_Basis_Functions();
            // copy local interpolation coordinates
            Omega_Interp_Data.Copy_Local_X(DoE_Pt,Vector_P1_phi_restricted_to_Omega.local_coord);
...
{
    Output_Single_Interpolation(0, Iobj_I_p->mxInterp_Data, mxCreateString(Iobj_I_p->Name), plhs[0]);
    delete(Iobj_I_p);
