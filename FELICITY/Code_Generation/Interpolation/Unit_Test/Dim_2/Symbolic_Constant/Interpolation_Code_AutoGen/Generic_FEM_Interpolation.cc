...
void GFI::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    f_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_f_Values], &Scalar_P2_phi_restricted_to_Omega);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the desired interpolations to evaluate
    Iobj_I_f = new I_f(Omega_Interp_Data.Num_Pts);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Iobj_I_f->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_f->c0_restricted_to_Omega = &c0_restricted_to_Omega;
    Iobj_I_f->f_restricted_to_Omega = &f_restricted_to_Omega;
...
void GFI::Evaluate_Interpolations ()
{
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            // copy local interpolation coordinates
            Omega_Interp_Data.Copy_Local_X(DoE_Pt,Const_Space_Omega_TupleSize_2_1_phi_restricted_to_Omega.local_coord);
            ...
            f_restricted_to_Omega.Compute_Func();

            // loop through the desired FEM interpolations
            Iobj_I_f->Eval_All_Interpolations(DoE_Pt);
            }
        }
...
{
    Output_Single_Interpolation(0, Iobj_I_f->mxInterp_Data, mxCreateString(Iobj_I_f->Name), plhs[0]);
    delete(Iobj_I_f);
