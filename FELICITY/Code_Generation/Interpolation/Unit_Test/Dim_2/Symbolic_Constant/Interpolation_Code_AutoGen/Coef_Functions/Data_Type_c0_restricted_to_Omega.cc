...
#define SpecificNAME       "c0"
#define SpecificFUNC        Data_Type_c0_restricted_to_Omega
#define SpecificFUNC_str   "Data_Type_c0_restricted_to_Omega"
...
public:
    ...
    const Data_Type_Const_Space_Omega_TupleSize_2_1_phi_restricted_to_Omega*     basis_func;
    ...
    // coefficient constant evaluated once (i.e. it is "read in" elsewhere)
    SCALAR Constant_C_Value[NC];

    // constructor
    SpecificFUNC ();
    ~SpecificFUNC (); // destructor
    void Setup_Function_Space(const mxArray*, const Data_Type_Const_Space_Omega_TupleSize_2_1_phi_restricted_to_Omega*);
    void Compute_Func();
private:
};
/*------------   END: Auto Generate ------------*/
...
SpecificFUNC::SpecificFUNC ()
{
    ...
    for (int nc_i = 0; (nc_i < Num_Comp); nc_i++)
        Constant_C_Value[nc_i].Set_To_Zero();
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFUNC::Setup_Function_Space(const mxArray* Values,        // inputs
                                        const Data_Type_Const_Space_Omega_TupleSize_2_1_phi_restricted_to_Omega*   basis_func_input)          // inputs
{
...
{
    // we read in the given constant elsewhere (and we only do it *once*)
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
