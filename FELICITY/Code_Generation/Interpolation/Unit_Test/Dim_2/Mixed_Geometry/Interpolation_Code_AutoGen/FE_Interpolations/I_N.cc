...
//
//    matrix([[geomGamma_N1], [geomGamma_N2], [geomGamma_N3]]), on Sigma.
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM interpolation (should be the same as the filename of this file)
#define SpecificFEM        I_N
#define SpecificFEM_str   "I_N"
...
public:
    ...
    const Data_Type_NN_restricted_to_Sigma*  NN_restricted_to_Sigma;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_00(double& INTERP)
{
    ...
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Sigma->Map_Normal_Vector[0].v[0];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_10(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Sigma->Map_Normal_Vector[0].v[1];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_20(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = geom_Gamma_embedded_in_Gamma_restricted_to_Sigma->Map_Normal_Vector[0].v[2];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
