...
//
//    matrix([[v_v1_t1], [v_v2_t1]]), on Omega.
//
/*------------   END: Auto Generate ------------*/
...
public:
    ...
    const Data_Type_v_restricted_to_Gamma*  v_restricted_to_Gamma;
...
void SpecificFEM::Eval_Interp_00(double& INTERP)
{
    ...
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = v_restricted_to_Omega->Func_vv_Value[0][qp].v[0];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Eval_Interp_10(double& INTERP)
{
    ...
    // only one point for interpolation
    for (unsigned int qp = 0; qp < 1; qp++)
          INTERP = v_restricted_to_Omega->Func_vv_Value[0][qp].v[1];
}
/***************************************************************************************/
/*------------   END: Auto Generate ------------*/
