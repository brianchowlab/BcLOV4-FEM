...
GFI::GFI (const mxArray *prhs[])
{
    ...
    Domain_Omega_embedded_in_Omega_restricted_to_Gamma.Setup_Data(prhs[PRHS_Omega_Mesh_Subdomains], prhs[PRHS_Omega_Mesh_DoFmap], PRHS_EMPTY);
    Domain_Omega_embedded_in_Omega_restricted_to_Omega.Setup_Data(prhs[PRHS_Omega_Mesh_Subdomains], prhs[PRHS_Omega_Mesh_DoFmap], PRHS_EMPTY);

    Gamma_Interp_Data.Setup("Gamma", 1, prhs[PRHS_Gamma_Interp_Data]);
...
void GFI::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    BDM1_phi_restricted_to_Gamma.Setup_Function_Space(prhs[PRHS_BDM1_DoFmap]);
    BDM1_phi_restricted_to_Gamma.Mesh = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    BDM1_phi_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_BDM1_DoFmap]);
    BDM1_phi_restricted_to_Omega.Mesh = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup access to external FE functions
    v_restricted_to_Gamma.Setup_Function_Space(prhs[PRHS_v_Values], &BDM1_phi_restricted_to_Gamma);
    v_restricted_to_Omega.Setup_Function_Space(prhs[PRHS_v_Values], &BDM1_phi_restricted_to_Omega);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // setup the desired interpolations to evaluate
    Iobj_I_on_Gamma = new I_on_Gamma(Gamma_Interp_Data.Num_Pts);
    Iobj_I_v = new I_v(Omega_Interp_Data.Num_Pts);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Iobj_I_on_Gamma->geom_Gamma_embedded_in_Omega_restricted_to_Gamma = &geom_Gamma_embedded_in_Omega_restricted_to_Gamma;
    Iobj_I_on_Gamma->geom_Omega_embedded_in_Omega_restricted_to_Gamma = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    Iobj_I_on_Gamma->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_on_Gamma->v_restricted_to_Gamma = &v_restricted_to_Gamma;
    Iobj_I_on_Gamma->v_restricted_to_Omega = &v_restricted_to_Omega;

    Iobj_I_v->geom_Gamma_embedded_in_Omega_restricted_to_Gamma = &geom_Gamma_embedded_in_Omega_restricted_to_Gamma;
    Iobj_I_v->geom_Omega_embedded_in_Omega_restricted_to_Gamma = &geom_Omega_embedded_in_Omega_restricted_to_Gamma;
    Iobj_I_v->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_v->v_restricted_to_Gamma = &v_restricted_to_Gamma;
...
void GFI::Evaluate_Interpolations ()
{
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            Domain_Gamma_embedded_in_Omega_restricted_to_Gamma.Read_Embed_Data(DoE_Elem_Index);
            Domain_Omega_embedded_in_Omega_restricted_to_Gamma.Read_Embed_Data(DoE_Elem_Index);

            // get the local simplex transformation
            // copy local interpolation coordinates
            Gamma_Interp_Data.Copy_Local_X(DoE_Pt,geom_Gamma_embedded_in_Omega_restricted_to_Gamma.local_coord);
            geom_Gamma_embedded_in_Omega_restricted_to_Gamma.Compute_Local_Transformation();
            // copy local interpolation coordinates
            Gamma_Interp_Data.Copy_Local_X(DoE_Pt,geom_Omega_embedded_in_Omega_restricted_to_Gamma.local_coord);
            ...
            // copy local interpolation coordinates
            Gamma_Interp_Data.Copy_Local_X(DoE_Pt,BDM1_phi_restricted_to_Gamma.local_coord);
            BDM1_phi_restricted_to_Gamma.Transform_Basis_Functions();

            // perform pre-computations with external FE coefficient functions
            v_restricted_to_Gamma.Compute_Func();

            // loop through the desired FEM interpolations
            Iobj_I_on_Gamma->Eval_All_Interpolations(DoE_Pt);
            }
        }
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            // NOTE: this must come before the external FE coefficient functions
            // copy local interpolation coordinates
            Omega_Interp_Data.Copy_Local_X(DoE_Pt,BDM1_phi_restricted_to_Omega.local_coord);
            BDM1_phi_restricted_to_Omega.Transform_Basis_Functions();
...
{
    Output_Single_Interpolation(0, Iobj_I_on_Gamma->mxInterp_Data, mxCreateString(Iobj_I_on_Gamma->Name), plhs[0]);
    delete(Iobj_I_on_Gamma);

    Output_Single_Interpolation(1, Iobj_I_v->mxInterp_Data, mxCreateString(Iobj_I_v->Name), plhs[0]);
    delete(Iobj_I_v);
