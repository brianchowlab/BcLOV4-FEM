...
// FE matrix is:
//
// \int_{dOmega}  psi_h_v1_t1*u_C_h_v1_t1*(u_M_h_coef_v1_t1 + v_M_h_coef_v1_t1)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_B_psi_Data_Type
...
public:
    ...
    const CLASS_geom_Omega_embedded_in_Omega_restricted_to_dOmega*  geom_Omega_embedded_in_Omega_restricted_to_dOmega;
    const CLASS_geom_dOmega_embedded_in_Omega_restricted_to_dOmega*  geom_dOmega_embedded_in_Omega_restricted_to_dOmega;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pointers for accessing FE basis functions
    const Data_Type_C_h_phi_restricted_to_dOmega*  C_h_phi_restricted_to_dOmega;
    const Data_Type_M_h_phi_restricted_to_dOmega*  M_h_phi_restricted_to_dOmega;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pointers for accessing coefficient functions
    const Data_Type_u_M_h_coef_restricted_to_dOmega*  u_M_h_coef_restricted_to_dOmega;
    const Data_Type_v_M_h_coef_restricted_to_dOmega*  v_M_h_coef_restricted_to_dOmega;
    /*------------   END: Auto Generate ------------*/
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor(const CLASS_geom_dOmega_embedded_in_Omega_restricted_to_dOmega&);
    /*------------   END: Auto Generate ------------*/
private:
};
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_dOmega_embedded_in_Omega_restricted_to_dOmega& Mesh)
{
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*M_h_phi_restricted_to_dOmega->Func_f_Value)[i][qp].a*(*C_h_phi_restricted_to_dOmega->Func_f_Value)[j][qp].a*(u_M_h_coef_restricted_to_dOmega->Func_f_Value[0][qp].a+v_M_h_coef_restricted_to_dOmega->Func_f_Value[0][qp].a);
