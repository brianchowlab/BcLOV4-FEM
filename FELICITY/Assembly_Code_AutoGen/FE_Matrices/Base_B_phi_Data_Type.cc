...
// FE matrix is:
//
// \int_{dOmega}  phi_h_v1_t1*u_C_h_v1_t1*(u_M_h_coef_v1_t1 + v_M_h_coef_v1_t1)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_B_phi_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*C_h_phi_restricted_to_dOmega->Func_f_Value)[i][qp].a*(*C_h_phi_restricted_to_dOmega->Func_f_Value)[j][qp].a*(u_M_h_coef_restricted_to_dOmega->Func_f_Value[0][qp].a+v_M_h_coef_restricted_to_dOmega->Func_f_Value[0][qp].a);
