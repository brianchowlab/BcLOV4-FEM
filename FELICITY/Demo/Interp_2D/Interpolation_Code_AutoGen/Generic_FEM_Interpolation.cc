...
void GFI::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    Iobj_I_grad_p_X = new I_grad_p_X(Omega_Interp_Data.Num_Pts);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Iobj_I_grad_p_X->geom_Omega_embedded_in_Omega_restricted_to_Omega = &geom_Omega_embedded_in_Omega_restricted_to_Omega;
    Iobj_I_grad_p_X->p_restricted_to_Omega = &p_restricted_to_Omega;
...
void GFI::Evaluate_Interpolations ()
{
    ...
    for (unsigned int DoE_Pt = 0; DoE_Pt < Num_DoE_Interp_Pts; DoE_Pt++)
        ...
        if (DoE_Elem_Index_MATLAB_style==0) // cell index is INVALID, so ignore!
            ...
            Iobj_I_grad_p_X->Eval_All_Interpolations(DoE_Pt);
            }
        }
...
{
    Output_Single_Interpolation(0, Iobj_I_grad_p_X->mxInterp_Data, mxCreateString(Iobj_I_grad_p_X->Name), plhs[0]);
    delete(Iobj_I_grad_p_X);
