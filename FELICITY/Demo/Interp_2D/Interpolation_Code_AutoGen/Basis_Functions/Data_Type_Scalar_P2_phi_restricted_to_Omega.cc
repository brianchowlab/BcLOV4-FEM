...
#define SpecificFUNC        Data_Type_Scalar_P2_phi_restricted_to_Omega
#define SpecificFUNC_str   "Data_Type_Scalar_P2_phi_restricted_to_Omega"
...
// geometric dimension = 2
    ...
    phi_Grad[0].Set_Equal_To(x_hat*4.0+y_hat*4.0-3.0, x_hat*4.0+y_hat*4.0-3.0);
    phi_Grad[1].Set_Equal_To(x_hat*4.0-1.0, 0.0);
    phi_Grad[2].Set_Equal_To(0.0, y_hat*4.0-1.0);
    phi_Grad[3].Set_Equal_To(y_hat*4.0, x_hat*4.0);
    phi_Grad[4].Set_Equal_To(y_hat*-4.0, x_hat*-4.0-y_hat*8.0+4.0);
    phi_Grad[5].Set_Equal_To(x_hat*-8.0-y_hat*4.0+4.0, x_hat*-4.0);
...
/*------------ BEGIN: Auto Generate ------------*/
    ...
    for (int qp_i = 0; (qp_i < Num_QP); qp_i++)
        ...
        for (int basis_i = 0; (basis_i < Num_Basis); basis_i++)
            {
            Mat_Transpose_Vec(Mesh->Map_PHI_Inv_Grad[0], phi_Grad[basis_i], Func_f_Grad[basis_i][qp_i]);
            }
        }
/*------------   END: Auto Generate ------------*/
}
