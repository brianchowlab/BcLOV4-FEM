...
#define SpecificFEM        Block_Assemble_BC_Matrix_Data_Type
...
public:
    ...
    void Add_Entries_To_Global_Matrix_Outlet (const Base_BC_Matrix_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_BC_Matrix_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
/*------------ BEGIN: Auto Generate ------------*/
/***************************************************************************************/
/* assemble a local FE matrix on the domain Outlet */
void SpecificFEM::Add_Entries_To_Global_Matrix_Outlet(const Base_BC_Matrix_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->Vector_P2_phi_restricted_to_Outlet->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vector_P2_phi_restricted_to_Outlet->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    COO_J_IV_Range[1] = 12; // end of range
