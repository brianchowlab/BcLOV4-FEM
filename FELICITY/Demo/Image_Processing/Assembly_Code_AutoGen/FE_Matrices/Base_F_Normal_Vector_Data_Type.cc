...
// FE matrix is:
//
// \int_{Sigma}  F_v1_t1*(geomSigma_N1*vv_v1_t1 + geomSigma_N2*vv_v1_t2)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_F_Normal_Vector_Data_Type
...
public:
    ...
    const Data_Type_Scalar_P1_phi_restricted_to_Sigma*  Scalar_P1_phi_restricted_to_Sigma;
    ...
    const Data_Type_F_restricted_to_Sigma*  F_restricted_to_Sigma;
    /*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Sigma_embedded_in_Sigma_restricted_to_Sigma& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = F_restricted_to_Sigma->Func_f_Value[0][qp].a*geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Normal_Vector[0].v[0]*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
            const double  integrand_1 = F_restricted_to_Sigma->Func_f_Value[0][qp].a*geom_Sigma_embedded_in_Sigma_restricted_to_Sigma->Map_Normal_Vector[0].v[1]*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
