...
// FE matrix is:
//
// \int_{Sigma}  uu_v1_t1*vv_v1_t1 + uu_v1_t2*vv_v1_t2
//
/*------------   END: Auto Generate ------------*/
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[j][qp].a*(*Vector_P1_phi_restricted_to_Sigma->Func_f_Value)[i][qp].a;
