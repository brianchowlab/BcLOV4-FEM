...
void SpecificFEM::Add_Entries_To_Global_Matrix_Gamma(const Base_Stiff_Matrix_Data_Type* Block_00)
{
    ...
    const int row_elem_index = Block_00->Vh_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vh_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->Vh_phi_restricted_to_Gamma->Mesh->Domain->Sub_Cell_Index;
    Block_00->Vh_phi_restricted_to_Gamma->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
