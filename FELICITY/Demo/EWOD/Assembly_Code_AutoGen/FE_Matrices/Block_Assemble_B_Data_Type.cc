...
#define SpecificFEM        Block_Assemble_B_Data_Type
#define SpecificFEM_str   "B"
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_B_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Omega (const Base_B_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_B_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Omega(const Base_B_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->Q_h_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->Q_h_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->V_h_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->V_h_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
    ...
    // write (I,J,V) data for Block_00->FE_Tensor_0, i.e. the (0,0) block
    // write the data directly
    COO_I[1] = Row_Indices_0[Local_Row_Ind[0]];
    COO_J[1] = Col_Indices_0[Local_Col_Ind[1]];
    COO_J_IV_Range[1] = 1;
    COO_V[1] = Block_00->FE_Tensor_0[Local_Col_Ind[1]*ROW_NB + Local_Row_Ind[0]];
    ...
    COO_J_IV_Range[2] = 2;
    COO_V[2] = Block_00->FE_Tensor_0[Local_Col_Ind[2]*ROW_NB + Local_Row_Ind[0]];
    ...
    COO_J_IV_Range[3] = 3;
    COO_V[3] = Block_00->FE_Tensor_0[Local_Col_Ind[3]*ROW_NB + Local_Row_Ind[0]];
    ...
    COO_J_IV_Range[4] = 4;
    COO_V[4] = Block_00->FE_Tensor_0[Local_Col_Ind[4]*ROW_NB + Local_Row_Ind[0]];

    // write column #5
    // write (I,J,V) data for Block_00->FE_Tensor_0, i.e. the (0,0) block
    // write the data directly
    COO_I[5] = Row_Indices_0[Local_Row_Ind[0]];
    COO_J[5] = Col_Indices_0[Local_Col_Ind[5]];
    COO_J_IV_Range[5] = 5;
    COO_V[5] = Block_00->FE_Tensor_0[Local_Col_Ind[5]*ROW_NB + Local_Row_Ind[0]];

    COO_J_IV_Range[6] = 6; // end of range
