...
// FE matrix is:
//
// \int_{Gamma}  mu_h_v1_t1*(geomGamma_N1*u_h_v1_t1 + geomGamma_N2*u_h_v2_t1)
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_C_Data_Type
...
public:
    ...
    const Data_Type_G_h_phi_restricted_to_Omega*  G_h_phi_restricted_to_Omega;
    const Data_Type_M_h_phi_restricted_to_Gamma*  M_h_phi_restricted_to_Gamma;
    const Data_Type_Q_h_phi_restricted_to_Omega*  Q_h_phi_restricted_to_Omega;
    ...
    const Data_Type_Y_h_phi_restricted_to_Gamma*  Y_h_phi_restricted_to_Gamma;
    /*------------   END: Auto Generate ------------*/
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    void Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma&);
    /*------------   END: Auto Generate ------------*/
private:
};
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = 0; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = (*M_h_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*(geom_Gamma_embedded_in_Omega_restricted_to_Gamma->Map_Normal_Vector[0].v[0]*V_h_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[0]+geom_Gamma_embedded_in_Omega_restricted_to_Gamma->Map_Normal_Vector[0].v[1]*V_h_phi_restricted_to_Gamma->Func_vv_Value[j][qp].v[1]);
