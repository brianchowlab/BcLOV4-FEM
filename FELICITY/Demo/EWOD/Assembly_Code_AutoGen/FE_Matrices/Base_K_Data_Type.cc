...
// FE matrix is:
//
// \int_{Gamma}  x_h_v1_t1_grad1*y_h_v1_t1_grad1 + x_h_v1_t1_grad2*y_h_v1_t1_grad2 + x_h_v1_t2_grad1*y_h_v1_t2_grad1 + x_h_v1_t2_grad2*y_h_v1_t2_grad2
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_K_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = Y_h_phi_restricted_to_Gamma->Func_f_Grad[j][qp].v[0]*Y_h_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[0]+Y_h_phi_restricted_to_Gamma->Func_f_Grad[j][qp].v[1]*Y_h_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[1];
