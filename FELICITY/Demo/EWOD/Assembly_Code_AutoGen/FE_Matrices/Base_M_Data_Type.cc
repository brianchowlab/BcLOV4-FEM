...
// FE matrix is:
//
// \int_{Omega}  u_h_v1_t1*v_h_v1_t1 + u_h_v2_t1*v_h_v2_t1
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_M_Data_Type
    ...
    for (unsigned int j = 0; j < COL_NB; j++)
        ...
        for (unsigned int i = j; i < ROW_NB; i++)
            ...
            for (unsigned int qp = 0; qp < NQ; qp++)
                {
                const double  integrand_0 = V_h_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[0]*V_h_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[0]+V_h_phi_restricted_to_Omega->Func_vv_Value[j][qp].v[1]*V_h_phi_restricted_to_Omega->Func_vv_Value[i][qp].v[1];
