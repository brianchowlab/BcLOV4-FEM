...
#define SpecificFEM        Block_Assemble_A_Data_Type
...
public:
    ...
    /*------------ BEGIN: Auto Generate ------------*/
    // constructor
    SpecificFEM (const PTR_TO_SPARSE*, const Base_A_Data_Type*);
    ~SpecificFEM (); // destructor
    void Init_Matrix_Assembler_Object(bool);
    void Add_Entries_To_Global_Matrix_Omega (const Base_A_Data_Type*);
    /*------------   END: Auto Generate ------------*/
private:
};
...
SpecificFEM::SpecificFEM (const PTR_TO_SPARSE* Prev_Sparse_Data,
                          const Base_A_Data_Type* Block_00
                          ) :
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Add_Entries_To_Global_Matrix_Omega(const Base_A_Data_Type* Block_00)
{
    // get local to global index map for the current ROW element
    int  Row_Indices_0[ROW_NB];
    const int row_elem_index = Block_00->G_h_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->G_h_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(row_elem_index, Row_Indices_0);
    ...
    const int col_elem_index = Block_00->G_h_phi_restricted_to_Omega->Mesh->Domain->Sub_Cell_Index;
    Block_00->G_h_phi_restricted_to_Omega->Get_Local_to_Global_DoFmap(col_elem_index, Col_Indices_0);
