...
// in this domain:  Sphere,
//                   with topological dimension = 2
//                    and   geometric dimension = 3
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the search domain (should be the same as the filename of this file)
#define SpecificSEARCH        CLASS_Search_Sphere
#define SpecificSEARCH_str   "CLASS_Search_Sphere"

// define optimization parameters
#define NUM_INTERMEDIATE_PTS  0
#define OPT_TOL  1E-8
#define BIAS_TOL  0.25
#define MAX_QUEUE_SIZE  200
#define MAX_GRADIENT_DESCENT_ITER  500
#define MAX_LINE_SEARCH_ITER  300
#define LINEAR_CELL
/*------------   END: Auto Generate ------------*/
...
public:
    ...
    CLASS_Domain_Sphere_embedded_in_Sphere_restricted_to_Sphere*  Domain;
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // access and compute local mesh geometry info
    CLASS_geom_Sphere_embedded_in_Sphere_restricted_to_Sphere*  GeomFunc;
    /*------------   END: Auto Generate ------------*/
    ...
    void Find_Points();
    bool Get_Valid_Neighbor(unsigned int&, int&, const int&,
                            const unsigned int*, const unsigned int&);
    bool Find_Single_Point(PT_DATA_Top2_Geo3&);
    bool Find_Single_Point_Sub_Iter(const VEC_3x1& GX, unsigned int& ci,
                                    unsigned int* ciq, unsigned int& qs,
                                    Min_On_Cell_Top2& M, int ln[2]);
    void Compute_Min_On_Triangle(const VEC_3x1& GX, const unsigned int& ci,
                                 const int& asi, const int& pmsi,
                                 unsigned int* ciq, unsigned int& qs,
                                 Min_On_Cell_Top2& M, int ln[2]);
    bool Run_Gradient_Descent(const VEC_3x1&, Min_On_Cell_Top2&, const double&);
    double Eval_Cost(const VEC_3x1&, const VEC_2x1&);
    void Eval_Cost_And_Gradient(const VEC_3x1&, const VEC_2x1&, SCALAR&, VEC_2x1&);
    void Eval_Cost_And_Directional_Deriv(const VEC_3x1&, const VEC_2x1&, const VEC_2x1&, SCALAR&, SCALAR&);
    void Bisection_And_Cubic_Line_Search(const VEC_3x1&, const VEC_2x1&, const unsigned int&,
                                         const double&, Min_On_Cell_Top2&, Min_On_Cell_Top2&);
    /*------------   END: Auto Generate ------------*/
private:
};
...
#undef NUM_INTERMEDIATE_PTS
#undef OPT_TOL
#undef BIAS_TOL
#undef MAX_QUEUE_SIZE
#undef MAX_GRADIENT_DESCENT_ITER
#undef MAX_LINE_SEARCH_ITER
#undef LINEAR_CELL
