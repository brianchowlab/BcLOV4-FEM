...
GPS::GPS (const mxArray *prhs[])
{
    ...
    Domain_Sphere_embedded_in_Sphere_restricted_to_Sphere.Setup_Data(prhs[PRHS_EMPTY_2], prhs[PRHS_Sphere_Mesh_DoFmap], PRHS_EMPTY);

    Sphere_Search_Data.Setup("Sphere", 3, prhs[PRHS_Sphere_Search_Data]);

    Sphere_Found_Points.Setup(&Sphere_Search_Data, 2);

    geom_Sphere_embedded_in_Sphere_restricted_to_Sphere.Setup_Mesh_Geometry(prhs[PRHS_Sphere_Mesh_Vertices], prhs[PRHS_Sphere_Mesh_DoFmap], prhs[PRHS_EMPTY_1]);
    geom_Sphere_embedded_in_Sphere_restricted_to_Sphere.Domain = &Domain_Sphere_embedded_in_Sphere_restricted_to_Sphere;
...
void GPS::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    Sphere_Search_Obj = new CLASS_Search_Sphere(&Sphere_Search_Data, &Sphere_Found_Points);
    /*------------   END: Auto Generate ------------*/

    /*------------ BEGIN: Auto Generate ------------*/
    // pass pointers around
    Sphere_Search_Obj->Domain = &Domain_Sphere_embedded_in_Sphere_restricted_to_Sphere;
    Sphere_Search_Obj->GeomFunc = &geom_Sphere_embedded_in_Sphere_restricted_to_Sphere;
    Sphere_Search_Obj->Consistency_Check();
...
{
    // find points in the sub-Domain: Sphere
    Sphere_Search_Obj->Find_Points();
...
{
    Output_Single_Point_Data(0, Sphere_Found_Points.Get_mxLocal_Points_Ptr(), mxCreateString(Sphere_Found_Points.Domain_Name), plhs[0]);
