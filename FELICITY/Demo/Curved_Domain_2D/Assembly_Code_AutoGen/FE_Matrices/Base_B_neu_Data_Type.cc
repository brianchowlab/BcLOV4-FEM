...
// FE matrix is:
//
// \int_{Gamma}  geomGamma_N1*v_h_v1_t1_grad1 + geomGamma_N2*v_h_v1_t1_grad2
//
/*------------   END: Auto Generate ------------*/

/*------------ BEGIN: Auto Generate ------------*/
// define the name of the FEM matrix (should be the same as the filename of this file)
#define SpecificFEM        Base_B_neu_Data_Type
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Omega_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = geom_Gamma_embedded_in_Omega_restricted_to_Gamma->Map_Normal_Vector[qp].v[0]*V_h_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[0]+geom_Gamma_embedded_in_Omega_restricted_to_Gamma->Map_Normal_Vector[qp].v[1]*V_h_phi_restricted_to_Gamma->Func_f_Grad[i][qp].v[1];
